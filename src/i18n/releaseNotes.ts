...
import { getCurrentReleaseNotes as getEagerCurrentReleaseNotes } from "./currentReleaseNotes";
...
const release125Copy = {
  ...
  replicationDescription: {
    "zh-CN": "复制终局产出前，建筑制造中心会按锁定的实际供电倍率递归消耗玩家真实库存并推进正在施工的项目；不会凭空生成施工材料。",
    en: "Before copied endgame outputs are awarded, construction centers recursively consume the player's real inventory and advance active projects at the locked runtime power factor. No construction material is fabricated.",
  },
  workerTitle: { "zh-CN": "模拟 Worker 故障可原地恢复", en: "Simulation Worker faults recover in place" },
  workerDescription: {
    "zh-CN": "运行异常、超时和 durable 恢复故障会退还尚未提交的挂机时间，重建 Worker 并从当前已提交状态继续；内容包校验错误与运行故障分离，不再污染自动恢复。",
    en: "Runtime failures, timeouts, and durable-recovery faults refund uncommitted idle time, rebuild the Worker, and resume from the current committed state. Content-pack validation errors remain separate and do not poison the runtime recovery path.",
  },
  rollbackTitle: { "zh-CN": "后台保护不再自动把存档回退", en: "Automatic safeguards never roll the save backward" },
  rollbackDescription: {
    "zh-CN": "内存保护、Worker 恢复、超时处理和纯挂机修复可以停止或重试，但不能安装历史权威检查点；旧检查点只允许玩家明确点击恢复时使用。",
    en: "Memory guards, Worker recovery, timeout handling, and pure-idle repair may stop or retry, but cannot install a historical authoritative checkpoint. Older checkpoints remain available only through an explicit player restore action.",
  },
  blueprintTitle: { "zh-CN": "密集蓝图详细信息不再错位重叠", en: "Dense blueprint details no longer overlap" },
  blueprintDescription: {
    "zh-CN": "蓝图详细卡片改用自然高度行布局和有界溢出；超大数量、长参数、端口列表以及放大界面字号时都能保持可读，不再相互覆盖。",
    en: "Detailed blueprint cards use natural-height rows and bounded overflow, keeping large counts, long parameters and port lists readable at dense desktop layouts and enlarged UI font scales.",
  },
  boundaryTitle: { "zh-CN": "存档与服务器格式保持兼容", en: "Save and server formats remain compatible" },
  boundaryDescription: {
    "zh-CN": "本版只调整纯挂机结算、运行时恢复和蓝图显示，不迁移玩家状态、云存档正文或生产数据库布局。",
    en: "This release changes pure-idle settlement, runtime recovery, and blueprint presentation only. It does not migrate player state, cloud payloads, or the production database layout.",
  },
} as const;
...
export function getCurrentReleaseNotes(locale: AppLocale): LocalizedReleaseNoteRecord {
  return getEagerCurrentReleaseNotes(locale);
}
...
export function getReleaseNotes125(locale: AppLocale): LocalizedReleaseNoteRecord {
  return {
    ...
    version: "1.2.5",
    title: release125Message(locale, "title"),
    summary: release125Message(locale, "summary"),
    items: [
      { id: "v125-conservative-continuity", title: release125Message(locale, "conservativeTitle"), description: release125Message(locale, "conservativeDescription") },
      { id: "v125-replication-construction", title: release125Message(locale, "replicationTitle"), description: release125Message(locale, "replicationDescription") },
      { id: "v125-worker-auto-resume", title: release125Message(locale, "workerTitle"), description: release125Message(locale, "workerDescription") },
      { id: "v125-no-automatic-rollback", title: release125Message(locale, "rollbackTitle"), description: release125Message(locale, "rollbackDescription") },
      { id: "v125-blueprint-detail-layout", title: release125Message(locale, "blueprintTitle"), description: release125Message(locale, "blueprintDescription") },
      { id: "v125-compatibility-boundary", title: release125Message(locale, "boundaryTitle"), description: release125Message(locale, "boundaryDescription") },
    ],
  };
}

export function getReleaseNotes124(locale: AppLocale): LocalizedReleaseNoteRecord {
  return {
