...
>
> **Pure-idle v10 证书准备纵切（2026-09-01）**沿用同一个 `DeterministicRuntime` 池，并把 ordinary 闭合流证书拆成两个固定 join 波次：第一波只读准备有限矿源候选、科研 sink、太阳帆 sink 与火箭 sink；完整 join 后，第二波只读准备依赖第一波私有结果的有限矿账本、仅在帆 sink 存在时启用的动态射线可再生电力证明，以及三窗口 ordinary flow。施工尾段另以同池并发准备永久可再生电网、制造中心身份和量子 fingerprint/带宽。每波必须全部 join，错误仍严格按历史 `source → finite vein → research → sail → renewable → rocket → flow/recipe` 和 `renewable → centers → quantum` 顺序选择；太阳帆仍优先于火箭，最终证书和所有共享库存/戴森写入仍串行。小目录、单 worker 和单一活动域自动串行，绝不跨时间并行，也不从 worker 修改权威 revision。
>
> 4,166 实体、1 条线路的无玩家数据合成目录在 8-worker 诊断中实际得到 wave1 `4/4`、wave2 `2/2`、construction `3/3` selected/observed worker；同一不可变输入的 release prepare-only 15 轮交错 A/B 从 1-worker 中位 `31,318,700 ns` 降至 8-worker `21,377,000 ns`（`-31.744%`），通过预先固定的 `3.5%` 接纳门禁。该数字只包含 ordinary 两波和 construction prepare，不包含 30 秒精确探针、尾段应用、提交、历史、序列化或磁盘 I/O，不能外推为完整纯挂机或玩家大档加速。原始样本、输入哈希、调度形状和公式记录在 `docs/releases/1.2.3-windows-native-pure-idle-certificate-parallel-evidence-2026-09-01.json`。
