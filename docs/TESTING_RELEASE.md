...
> **当前发布基线（2026-08-11）**：当前正式版本仍使用 `1.0.36 / GameState v46`，本地开发候选为 `1.0.38 / GameState v46`；有效资产的 v43 空间站实验存档拒绝加载。量子网络回归以 GameState v46 语义为基线，并保留传统物流站升级兼容测试。1.0.38 不升级存档 envelope、云 schema 或 SQLite layout。

> `pure-idle-macro-v4` 隔离热修候选在 2026-08-11 通过 typecheck、Vitest 107 文件通过/6 文件跳过且 956 项通过/18 项跳过、生产构建，以及 Playwright 280 项通过/12 条显式条件夹具跳过/0 失败（292 总数，829.9 秒）。玩家提供的 2,148,602 字节导出档以外部只读 fixture 单独执行 19 项相关 E2E：18 项通用恢复/UI 通过、1 项玩家档专测通过、1 项未配置的另一真实终局夹具跳过；该档在 35/40/60/120/360 模拟秒五个窗口内，分段精确路径与普通模拟的逐物品累计产量相对误差均为 0，源哈希和正式重载校验保持不变。当前 1.0.38 发布 Agent 的源码、制品和端口均未替换；本结果只属于独立热修分支。

> `1.0.38` 在 2026-08-11 完成开发门禁：typecheck、Vitest 106 文件通过/6 文件跳过且 948 项通过/18 项跳过、服务端 70/2、运维 6/6、原生工具 8/8、128 个运行时许可证，以及 Playwright 280 项通过/11 条显式条件跳过/0 失败（291 总数，838.6 秒）。根项目与服务端生产依赖审计均为 0。专项覆盖 Worker 可转移保存/槽位/快照、可信信封拒绝、稀疏 v46 往返、持久传送带账本复用、量子原地/不可变等价、单极磁石一→二硬上限与零物资、200% 桌面/移动 Canvas 有界矩阵；1.0.35 的纯挂机与模式隔离、1.0.36/1.0.37 的传送带、科技树、离线决策和星图批量操作继续全量执行。

> `1.0.37` clean source `853ecdb12795844c484b1415f8e72967a25e343d` 在 2026-08-11 完成发布前门禁：typecheck、Vitest 103 文件通过/6 文件跳过且 936 项通过/17 项跳过、服务端 70/2、运维 6/6、原生工具 8/8、128 个运行时许可证，以及 Playwright 277 项通过/11 条显式条件跳过/0 失败（288 总数，803.5 秒）。根项目与服务端生产依赖审计均为 0。新增矩阵覆盖单极磁石 v20+ 资源目录迁移/人工修复闭环、离线 decision-required/取消/精确重试/普通零收益跳过、科技树标准/紧凑 × 100/150/200% 与多输入方式、星图批量物流宽窄/手机/200% 及成功/跳过原因；1.0.35 的纯挂机与存档模式隔离、1.0.36 的传送带与终局性能套件继续全量执行。生产仍保持 1.0.36。
...
> 正式发布以源码 `e0ad49062fa329040b379375b595ba74b7d23daf` / Build ID `1.0.36+e0ad49062fa3` 完成两地备份、未激活目录、备份副本隔离启动、原子切换、9 文件公网下载、APK/EXE 完整哈希、Range/cache、当前/上一版资源和五场 Chrome smoke。当前 PWA Cache Storage/离线恢复与上一稳定版 1.0.35 worker 隔离均通过。Android 使用批准长期证书；Windows 为 `NotSigned`。用户只豁免该候选的 Android 真机、低配 Windows 和覆盖升级门禁，不能替代实机结论。生产证据见 [releases/1.0.36.md](./releases/1.0.36.md)。
...
| 单元/领域 | `npm test` | 隔离热修候选：107 个文件通过、6 个跳过；956 项通过、18 项跳过 | 引擎暂停边界、递归制造守恒、Worker/稀疏存档、单极磁石扩容、科研宏观账本、离线决策、纯挂机幂等/守恒/不稳定速率回退、模式存档隔离、v1-v46 存档、传送带/物流索引 oracle 和云同步等 |
| 浏览器 E2E | `npm run test:e2e` | 隔离热修候选：280 项通过、12 项显式条件夹具跳过、0 失败 | Worker 槽位/快照、矿脉扩容、Canvas 有界矩阵、科技树横向输入/多字号、星图批量物流、离线决策、版本公告、速通、纯挂机 v4/旧恢复日志/玩家档、模式隔离、新旧手机壳和 IndexedDB 回归 |
| 云服务 | `npm run test:server` | `1.0.36`：70 项通过、2 项可选夹具跳过 | 用户名注册、模式化云槽、schema v3→v7、SQLite layout v1→v2、v35-v46 边界、治理指标、账号安全、历史恢复保护、排行榜完整性和管理员保护 |
...
| 生产构建 | `npm run build` | 1.0.38 source freeze 后执行 clean build | `tsc -b`、Vite chunk、普通离线/宏观 Worker 和 PWA 资源；生产多 Worker 入口硬关闭 |
| 桌面目录包 | `npm run desktop:pack` | 1.0.38 source freeze 后只生成未签名诊断目录包 | Electron 启动、包内版本与 Windows 解包 |
| Android release | `npm run android:release:unsigned` | 1.0.38 / 1000038；source freeze 后只生成 unsigned APK/AAB | applicationId、versionCode、SDK、zipalign 和签名缺失确认 |
...
1.0.37 的开发候选、真实大档性能边界、unsigned 原生诊断包和 Release Agent 条件交接见 [1.0.37 开发报告](./DEVELOPMENT_REPORT_1.0.37.md) 与 [1.0.37 Release Agent 交接](./RELEASE_HANDOFF_1.0.37.md)。该记录不代表生产已经发布。

1.0.38 的 Worker/批处理/存档瘦身、单极磁石一→二功能、真实大档证据和条件交接见 [1.0.38 开发报告](./DEVELOPMENT_REPORT_1.0.38.md) 与 [1.0.38 Release Agent 交接](./RELEASE_HANDOFF_1.0.38.md)。该记录不代表生产已经发布。

1.0.38 当前发布源码冻结后发现的时间扭曲纯挂机热修只存在于独立分支，见 [1.0.38 时间扭曲纯挂机热修交接](./RELEASE_HANDOFF_1.0.38-timewarp-save-fix.md)。发布 Agent 不得把尚未提交、未生成独立清单的工作树直接混入正在发布的 1.0.38 制品。
