...
> **1.1.9 建筑制造中心零材料耗时专项（2026-08-26，本地开发，未部署）**：递归材料步骤改为 0 秒即时原子结算，最终“建筑成品”仍按科技保持 5/2.5/1 秒周期，载具入库保持 0.01 秒；状态进度与产能查询对零时长步骤显式返回有限值。相关领域回归 265/265、完整 Vitest **188 文件通过 / 10 跳过；1,526 项通过 / 23 跳过**，TypeScript、生产构建和 startup budget 通过（1,973 modules；startup gzip **179,833 B**），隔离开发服务的实时 Worker 建筑制造 E2E 1/1 通过。一次误用 production preview 运行依赖 `/src` 动态导入的源码 E2E，因测试模块不可访问而失败；按该用例要求切回开发服务后通过，不属于产品行为失败。玩家原始存档 `dsp-idle-save-2026-08-26.json` 只读迁移验证中，同一座 **12,200,000** 堆叠制造中心在 1 秒最终周期、满功率和内存内补足原矿的条件下，1 模拟秒精确结算 **12,200,000 个最终建筑作业**，guard hit 为 0；原文件前后 SHA-256 均为 `F4D680C86B5528207753A96BA06DF2B396AF652C01DA7C6E18DFB6C2E6551EE8`。本段只记录本地开发证据，不表示已推送或部署。
