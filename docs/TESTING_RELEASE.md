...
> **2026-09-09 空星球模拟候选**：`2501f0d4` 定向 35/35、实际类型检查通过；随后完整本机 3,160 通过 / 39 跳过 / 1 失败，轻量 Windows 云端 34 通过 / 0 跳过 / 1 失败。递归量子建设案例分别 5,110.045 / 5,232.447 ms，仍超过默认 5 秒；有限矿脉原 2 秒断言两轮通过。完整云端仍在进行，历史失败保留，不放宽门槛。后续桌面测试只用已验证的隐藏、禁止聚焦、静音离屏入口；本机重任务低优先级串行，余量不足暂缓，禁止关闭用户进程腾资源。详见[结果记录](./reviews/rust-rp1-js-validation-cost-2026-09-09.md)与[后台约束](./reviews/rust-rp1-background-testing-2026-09-09.md)。
