...
describe("recipe focus thin read model", () => {
  ...
  it("routes the always-mounted panel through the bounded viewport base", () => {
    ...
    expect(panel).toMatch(/data-read-only=\{readOnly \|\| undefined\}/);
    expect(panel).toMatch(/disabled=\{readOnly\}/);
    expect(panel).toMatch(/if \(readOnly\) return;[\s\S]*?onPositionChange\(positionRef\.current\)/);
    expect(panel).not.toMatch(/GameState|game\.recipeFocus/);
    const workspace = readFileSync(resolve("src/components/RecipeWorkspace.tsx"), "utf8");
    expect(workspace).toMatch(/disabled=\{readOnly\}[\s\S]*?Windows 原生模式下暂不可修改聚焦状态/);
  });
});
