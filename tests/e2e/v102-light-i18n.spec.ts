...
const RELEASE_NOTE_ID = "2026-08-04-v1.0.26";
...
test("English light release notes are localized and persist dismissal", async ({ page }) => {
  ...
  await page.goto("/?menu=1&lang=en");
  const dialog = page.getByRole("dialog", { name: "Light Theme & Settings Interaction Update" });
  await expect(dialog).toBeVisible();
  await expect(dialog).toContainText("1.0.26");
