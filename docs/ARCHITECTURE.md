...
- `src/game/pureIdleMacro.ts`、`pureIdleMacro.worker.ts`、`pureIdleMacroClient.ts`：`pure-idle-macro-v5-lite` 终局宏观纯挂机的校准合同、候选状态、科研账本、验证摘要和正式重载门禁。启动及恢复从检查点重新计算时间扭曲供电倍率；普通合同不可用、影子尾验偏差、复杂度保护或 Worker 重试时切换有界保守宏观。保守会话从原始合法检查点建立 3 × 10 秒精确轻量校准；候选在墙钟跨过校准边界前仍停留在原检查点，跨过后原子安装唯一校准副本，再以常数时间应用普通生产合同。标准/受限/低内存设备的保守操作现实预算分别为 90/120/180 秒，实际完成即返回，不固定等待。Worker 代次继续隔离迟到消息。`pureIdleRecovery.ts` 将检查点、心跳、墙钟进度、失败次数、开始前暂停状态、结算 ID、检查点指纹、冻结边界、退出原因和提交标记保存到独立 IndexedDB；`inspectPureIdleRecovery()` 明确区分 live、committed、missing、invalid、unavailable，checkpoint 指纹防止把其他槽位或云档接到本地主时间线。Web Lock 与可过期租约防止重复结算，旧 schema v1 记录按读取时默认值兼容，恢复日志不属于 `GameState`、存档 envelope 或云 payload。
