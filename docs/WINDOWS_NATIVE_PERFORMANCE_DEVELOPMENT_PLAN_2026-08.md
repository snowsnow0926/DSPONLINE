...
| WIN-410 活动脏块保存 | 开发实现完成 | base、实体页、线路页、两类拓扑分别记脏；清洁页复用已验证 metadata；提交失败不清脏，durable commit ACK 后才清除 | 真实磁盘满、只读目录、Defender 锁文件和覆盖升级仍需独立 Windows 矩阵 |
| WIN-420 事件驱动线路 | 开发实现完成 | 路由组 active queue、闭合生产/库存/物流/电力唤醒、稳定顺序、稠密自动全扫描及可观测计数；合成休眠档与 full-scan oracle 在 1/5/60 秒严格一致 | 真实极限档大多数线路确实活跃，本次正确退化全扫描，因此该档没有伪造“跳过 90%”收益；还需 24 小时组合长跑 |
| WIN-430 确定性原生多核 | 部分完成 | 冷启动记录解析和稠密记录编码已有稳定分片并行；小批量自动单线程 | 权威模拟阶段仍是单线程确定性提交；没有 1/2/4/8 线程完整状态哈希矩阵，禁止标记完成 |
| WIN-440 有界增量投影 | 开发实现完成，Gate C 待测 | `viewport-v1`、`statistics-v1`、最多 1 MiB 二进制 MessagePort 块、session/revision/sequence/长度/SHA-256 双侧校验和 ACK | Electron MessagePortMain 当前是有界二进制 clone，不是共享内存零拷贝；全帧 IPC 占比与真实薄 UI 安装成本待测 |
...
| WIN-470 流式 v47/云准备 | 导出开发完成，整体部分完成 | Rust 直接流式写 envelope v2/GameState v47，同时计算旧 checksum、字节数和 SHA-256；renderer 不接收完整正文 | 原生流式导入和实际云文件句柄上传未闭合；生产云往返不在开发角色授权范围内 |
