...
fn peer_matches(
    ...
    for peer_index in station_indices(entities) {
        ...
        if string_at(peer, "buildingId") != Some("planetary_logistics_station")
...
pub(crate) fn admission_reason(state: &CoreState) -> anyhow::Result<Option<&'static str>> {
    ...
    for station_index in station_indices(&entities) {
        ...
        if string_at(station, "buildingId") != Some("planetary_logistics_station") {
...
pub(crate) fn transfer_buffers(
    ...
    for station in entities
        .iter_mut()
        .filter_map(Value::as_object_mut)
        .filter(|entity| string_at(entity, "buildingId") == Some("planetary_logistics_station"))
    {
...
fn update_congestion(entities: &mut [Value]) -> anyhow::Result<()> {
    let indexes = entity_index(entities);
    let ledger = build_ledger(entities, &indexes);
...
pub(crate) fn settle(
    ...
    dispatch(state, base, entities, powers)?;
    advance_routes(entities, seconds, powers)?;
    update_congestion(entities)
}
