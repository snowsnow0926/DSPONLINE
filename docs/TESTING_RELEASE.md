...
> **Windows 隔离会话验证（2026-09-10）**：新矩阵覆盖固定夹具漂移、Windows 实际目录/文件锁、独立 Host/main 助手身份、复制/替换、夹具篡改/硬链接/junction、受控参数、畸形响应及并发/终止失败。当前执行结果集中于[本批记录](./reviews/rust-windows-validation-session-2026-09-10.md)，不把历史候选结果计为新源码通过；原测试门槛、后台静音约束与完整发布缺口保留。
