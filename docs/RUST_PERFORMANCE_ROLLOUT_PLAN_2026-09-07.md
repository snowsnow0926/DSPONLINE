...
> **2026-09-10 用户收回当前范围：先按性能优化标准开发。** 当前优先 Windows 大存档返回游戏、离线等待、保存和内存成本；复用已实现的 Rust 热点路径和原界面。完整实时接管、完整引擎替换及其专用资格基础建设暂停推进，保留已完成代码和长期路线，不作为本轮性能成果的交付前提。以下扩大目标及阶段状态是历史记录，以本段和下一节为当前口径。工具当前无 Goal，本次不自动重建完整替换 Goal。

## 当前性能批次：大存档返回游戏

2026-09-10 执行结果：已取得当前完整 Native 请求的分段计时与三对样本。逐字节证明缓冲候选未证明稳定收益，已撤回；只保留可选诊断，下一步针对状态准备和结算前后的整档处理。当前尚无新的完整界面等待或整版提速结论，详见[测量及失败记录](./RUST_WINDOWS_FULL_PROGRESS_2026-09-10.md)。
...
- 计划 ID：RUST-PERF-20260907，版本 1.8；当前范围以顶部 2026-09-10 性能批次为准。
