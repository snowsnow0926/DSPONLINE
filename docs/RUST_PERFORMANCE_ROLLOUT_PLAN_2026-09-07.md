...
- 计划 ID：RUST-PERF-20260907，版本 1.4。
- 编制日期：2026-09-07；依据代码提交：`f7c907408bf2a3500368ef80abd71e111670b85c`。
- 执行状态：**独立原生侧车恢复、共享保存/导入及离线校验/准备阶段已验收，普通 Rust 离线边界正确性已验证；RP0 总矩阵 / RP1 完整离线收益未完成；Rust 实时玩家资格关闭。**
- 主要平台：Windows 性能开发版，保留现有 Electron/React 界面。
- 后续平台：Web/PWA、Android 复用 Rust 核心的可行性评估，单独排期。
- 2026-09-07 最新资格进展：`ab01bb78` 修复普通离线容量/矿量边界和无出口窗口，公共六例物料及关键进度 6/6、完整状态 3/6；后三例一致来自有预算的精确回退，不计为新性能收益。无证书/超预算/未证明时钟明确拒绝，长 WAL 增加算法与 v2 kind 保护，见 [Rust 离线边界记录](./reviews/1.2.7-rust-offline-boundaries-2026-09-07.md)。
...
- RP1 下一步：为普通宏的累计传输、生产历史/时间和量子运行边界建立原引擎等价更新，补一般小数源宏时钟与复合产线验证，再做真实入口验收。长时资格仍未通过；不能把部分字段相同或精确回退通过视为整体宏结算提速。
...
| RP1：离线收益闭环 | P1 | 可重复验证的 Rust 离线加速候选，含取消/回退/重开证据 | 3–7 人日 | IN_PROGRESS，独立准备收益及普通边界已验证，整体未完成 |
...
| ID | 当前状态 | 完成证据 | 下一动作 |
| --- | --- | --- | --- |
| PLAN | MAINTAINED，1.4 | 独立收益、边界资格及制品报告已分别登记 | 按实际验证维护状态与链接 |
| RP0 | IN_PROGRESS | 原生恢复、共享保存/导入及决定等待已有独立测量；总矩阵未完成 | 补完整离线/实时路由、不同硬件与全进程内存矩阵 |
| RP1 | IN_PROGRESS | 独立准备等待缩短 16.1%；Rust 公共六例物料/关键进度 6/6，三例精确回退全状态一致；整体资格未通过 | 补普通宏传输/历史/量子运行字段、一般小数源及复合路径，再验真实入口 |
...
下一次启动开发时，以 **RP1 剩余普通宏状态差分** 为首个有界任务，并继续保留 RP0 总矩阵缺口：

> 从 ab01bb78 的公开六例资格记录继续，优先为仍使用宏的三例补累计传输、生产历史/时间与量子运行边界的等价更新；不要直接改时间戳或乘最后一帧流量隐藏差异。保持精确回退预算、原子拒绝、版本化 WAL 和自动采用门禁，补一般小数源及复合产线后再验真实入口。代码与差分证据提交后，只有完整结果可重复成立才做同口径性能 A/B；不能把精确回退或决定弹窗等待当作 Rust 长时加速。独立恢复/保存/准备阶段的历史收益不重测、不相加，发布另行消费冻结候选与授权。
