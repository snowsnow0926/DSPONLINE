...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY.map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 27 }, (_, index) => 26 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 2).map((entry) => entry.version)).toEqual(["1.0.26", "1.0.25"]);
    expect(getReleaseNotesPage(12, 2).map((entry) => entry.version)).toEqual(["1.0.2", "1.0.1"]);
    expect(getReleaseNotesPage(13, 2).map((entry) => entry.version)).toEqual(["1.0.0"]);
  });
});
