...
test("English light release notes are localized and persist dismissal", async ({ page }) => {
  ...
  const dialog = page.getByRole("dialog", { name: "Quantum Supply and Automatic Construction Fixes" });
  await expect(dialog).toBeVisible();
  await expect(dialog).toContainText("1.3.0");
  await expect(dialog).toContainText("Stocked materials can be delivered");
  await expect(dialog).toContainText("Fair shared downloads");
  await expect(dialog).toContainText("Use ready-made ingredients");
  await expect(dialog).toContainText("Continue other ready targets");
  await expect(dialog).toContainText("Clear delivery status");
