...
| 单元/领域 | `npm test` | 香港 `0.7.0` 基线 282 项；上海 `0.5.0` 基线 268 项 | 引擎、v1-v31 存档、双向物流载具归属、制造中心递归任务、堆叠容量、物流槽自动配置、存档配额急救、四向分流、科研、电力、蓝图、规划、网络与云同步等 |
| 浏览器 E2E | `npm run test:e2e` | 香港 `0.7.0` 基线 113 项；上海 `0.5.0` 基线 97 项 | 从开局到银河终局、主题、逐行星视角、工作区再次关闭、生产资料库、新旧手机壳、保存失败和桌面/手机横竖屏回归 |
