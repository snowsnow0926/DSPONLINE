...
> **Windows 持续会话源码验证（2026-09-10）**：V1 正常优化 Host 286/4 ignored、两个入口各 3 与严格 Clippy；客户端退出修复后 V2 重新验证 Node 189、Native 966/1 skip、前端 7/类型及格式，逐文件及重建字节确认 Rust 未变。原 15 秒失联与真实 17 秒自动续期均通过，产品负例修复前失败/修复后通过。待冻结包及实际 ASAR 入口验证；原始失败、完整发布缺口均保留，见[本批记录](./reviews/rust-windows-validation-lease-2026-09-10.md)。
