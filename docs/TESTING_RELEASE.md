...
> **交接与连续产线专项（2026-09-10）**：native:realtime-rpc 现在从 renderer 所有的会话进入实际协调器、产品 IPC 适配/校验、preload 订阅和租约策略，再转移给 main/Rust；修复前入口失败，修复后原 62 秒完整产线科研、建造重试、暂停计时、丢失持久确认和三次重开全部保持。每阶段比较完整 JSON/摘要，三次实际 Rust 恢复记录与浏览器租约策略核对。Electron 事件及浏览器存储是合成环境，仍为 TEST_ONLY，不能替代实包/IndexedDB/完整准入验收。当前与历史证据见[进度报告](./RUST_WINDOWS_FULL_PROGRESS_2026-09-10.md)。
