...
>
> 本纵切冻结前 fresh focused：Core `3/3`、Host exact lease `1/1`、Host bin `3/3`、Node `62 passed / 6 skipped / 0 failed`、Vitest `12/12`；typecheck、`cargo fmt --check`、workspace all-target/all-feature `clippy -D warnings`、production build、startup budget 与 `git diff --check` 通过。6 个 Node skip 都明确因为本工作树尚未构建 release native Host，不是静默忽略失败。首轮 renderer channel 清单仍为新增前的 62 而报 `1 fail`，补入两个 exact IPC 后以 64 个双侧集合相等闭合；Rust fmt 首轮发现新增代码机械排版差异，执行 formatter 后复验通过。组合全量仍由根工作树统一执行。
...
> - [x] native import 的粘贴/文件 ingress 在解析前限制 well-formed/fatal UTF-8 与 1 MiB；renderer 只传 raw，不得 `JSON.parse`、检查目录或派生蓝图字段。Rust 必须独占 strict parse/canonicalize，限制 ordinary 内置域、512 entities、1,024 belts、64 library rows，library full 时失败且不得 eviction。
> - [x] import context 必须绑定同一 session/run/current revision、registry fingerprint、raw byte count/SHA-256 与活动陆地行星；唯一 marker 必须精确为 `{kind:"import",sourceName,blueprint,blueprintSha256,revision}`。raw exchange 禁止进入 command/WAL；prepared canonical blueprint、command 和 context 均受 1 MiB budget 约束，未知键、摘要漂移、目录/allocator/library 漂移必须在写入前拒绝。
> - [x] import 成功只能在一个原子候选中追加一条 revision 1 蓝图并更新 `nextId`/authority revision；不得改实体、线路、队列或驱逐旧蓝图。Host 必须证明 live、generic cold-WAL reopen 和 `AfterStage/AfterWal/AfterCheckpoint/AfterReceipt/AfterLeaseAcknowledge` 五边界状态/hash 一致且 command ID 幂等。UI 只能一次 mutation，unknown outcome 只按 `0/100/250/500/1000/2000 ms` 六次读 receipt，并以同 lineage/registry、当前 authority revision、目标 ID `present=true` 的 membership proof 收尾。
> - [x] native export 必须是同 revision/registry/选中行绑定的 Rust-only、read-only context。Rust 在 clone/serialize 前做有界图扫描，限制 512/1,024 和 1 MiB，确定性规范 node/line key、nullable optional 字段、belt 默认值与 Windows 文件名，生成无时间戳 v2 raw exchange、byte count 和 SHA-256；exchange/projection 任一越界或行/目录/语义漂移均失败且不写 state/WAL。
> - [x] desktop export 只验证 exact shape/binding/limits/byte count/SHA-256/文件名并下载 Rust raw；不得解析、重序列化或修补正文。读取期间 lineage、revision、registry、选中 ID/revision/name 漂移必须丢弃结果。native import/export 都不得调用 Web `parseBlueprintExchange`、`serializeBlueprintExchange` 或 `importBlueprintExchange` 作为权威 fallback。
> - [x] Web/PWA fallback 独立保留 JavaScript exchange，硬门限为 512 entities、1,024 belts、1 MiB raw/serialized exchange、64 library rows；第 65 条导入必须返回原状态，不得 eviction，也不得进入 Rust command/Host WAL。
> - [x] 最终组合源码的 focused evidence：capture/import/export UI `190/190`、相关 Node `58/58`、fresh Release Host integration `6/6`；独立 import+export IPC `9/9` 是 Node 子集，不重复相加。Rust 全量为 Core `833/833`、Host library `203/203`、Host main `2/2`，合计 `1038/1038`；Windows native/desktop 为 `491` 通过、`1` 个 Windows symlink 权限条件跳过、`0` 失败。
> - [x] **集成代码门禁已在最终组合源码新跑**：开启 long differential 的完整 Vitest 为 371 files（358 通过、13 条件跳过）、2,879 tests（2,851 通过、28 条件跳过、0 失败，432.19 秒）；TypeScript typecheck、Rust fmt、workspace strict clippy `-D warnings` 与 diff check 通过。production build 为 2,088 modules，startup 总 gzip `180,576 B`、JavaScript `86,989 B`、CSS `93,587 B`、最大启动 JavaScript `58,974 B`、menu `257,943 B`、forbidden startup module `0`。完整 Chromium 为 460 项（433 通过、27 条件跳过、0 失败，6.6 分钟）；durable WAL E2E 为 `7/7`、0 跳过、0 失败（48.7 秒）。desktop pack/install/覆盖升级、24 小时、多硬件、真实 Defender/磁盘、签名与灰度仍是独立未执行的正式发布 No-Go 门禁。
>
> 失败史必须保留：capture Core `1/4` 与 Host `193/196` 是 synthetic catalog/ordinary-domain fixtures，preload `7/8` 是 lone high surrogate 的 NaN 判断；修复边界见 24.20。import UI focused 首轮 `16/17` 是 stale identity 读取时序预期错误，修测试后 `20/20`。App/组件曾为 `52/53`，后为 `90/92`，原因分别是旧 integration regex 漏新增 pending，以及 fixture 将真实“模组蓝图 Ω”误写为“模块蓝图 Ω”；只修测试预期后，当前 UI focused 合并桶为 `190/190`，产品行为未为此放宽。Node import 曾为 `35/36`，旧 handler count 56 落后于当时 main/preload 的 57 且实际集合一致；按最终 import+export handler 集合修测试后，相关 Node focused 为 `58/58`，独立 IPC `9/9` 是其子集。Web fallback 首轮 `14/16` 来自 exact-1-MiB fixture 按字符而非 UTF-8 bytes 计数、长名 fixture 未形成真实碰撞，修 fixture 后 `16/16`。Rust export oversize fixture曾先命中 catalog invalid/catalog-incomplete，后拆成真实 serializer 边界与 projection 门；`MAX_SAFE` 分支误改与 clippy `items_after_test_module` 也已纠正。跨域 allocator 红测曾为 `0/1`，补齐 construction queue/version 域后通过；fresh Host 冷回放曾以 `prepared body not canonical` 揭露 Node `-0` 与 Rust `0` 词法漂移，统一规范整数零后 `6/6`。审计期间 export 一度误把合法 `version.blueprintId`/`queue.blueprintId` 外键当 owner 冲突，真实 import→enqueue→export 正向回归阻止该回归并修正为只拒绝独立 owner 复用。首次完整 Vitest 为 `2850` 通过、`28` 跳过、`1` 失败，唯一失败是 Windows 真实进程 benchmark 沿用全局 5 秒上限；测试自身允许两次 5 秒等待及清理，专属上限改为 20 秒后 focused `5/1/0`、最终全量 `2851/28/0`。并行 Release Host 构建还曾由 Windows rustc 触发 `STATUS_HEAP_CORRUPTION`；保持源码不变、`CARGO_BUILD_JOBS=1` 后成功，故单 Cargo job 仍是本机开发门禁。
>
> GameState v47、envelope v2、cloud schema v8、SQLite layout v3、package 1.2.3 与 `authorityEligible=false` 均不变；本门禁不授权版本、schema、生产、部署、签名或发布变更。

> **Windows Rust ordinary 蓝图捕获门禁（2026-08-31，开发候选）**：capture context 必须精确绑定同一原生权威 session/run/current revision、registry fingerprint、活动行星和未截断且保持顺序的 pinned entity IDs。Core 的 `blueprint-capture-context-v1` 只能返回 support/reason、预期 blueprint ID/name/revision 与公开 limits；合法 durable marker 只能是 `{kind:"capture",entityIds:[...],revision}`，夹带 session/run、planet、blueprint body/ID/name、entity/belt body、目录、`nextId` 或 fallback 字段必须在写入前拒绝。MOD/命名空间内容、resource anchor、external port、station/特殊建筑、目录不完整、position overlap、library full、ID 冲突/耗尽和 stale context 都必须保持源 revision/hash/entity/belt/queue/library 不变。
>
> Host 门禁必须证明 live、generic cold-WAL reopen 和 `AfterStage/AfterWal/AfterCheckpoint/AfterReceipt/AfterLeaseAcknowledge` 五个故障边界的完整状态/canonical hash 一致，WAL 只保存 capture marker，同一 command ID 不能重复新增蓝图。renderer 每个事务只允许一次 mutation；unknown transport 只能按 `0/100/250/500/1000/2000 ms` 做六次 main-owned receipt 只读 reconciliation，不能 resend 或调用 legacy `createBlueprint` / `commitGame`。ACK 必须严格为连续 `R+1`、空 changed entity/belt IDs 与 `topologyDirty=true`；最终还必须用同 lineage/registry 的 `library-membership`、目标 ID、cursor `0`、limit `32` 和 exact `{id}` row 证明成员存在。证明完成后只进入定位；实际落点必须另取 fresh direct-deploy context，不能把 capture context 当部署凭证。
>
> preload ingress 是本门禁的一部分：direct invoke 请求必须 exact-key，binary transfer 的 `{sessionId,projectionType,payload}` 外层和 capture payload 也必须 exact-key；entity IDs 必须为 1～512 个唯一、Unicode well-formed、无控制字符且各不超过 512 UTF-8 bytes，序列化请求不超过 1 MiB。未知键、重复、lone surrogate 或越界请求必须在 `ipcRenderer.invoke/postMessage` 前本地拒绝并证明发送次数为零；capability 缺失、broker/renderer binding 不一致和 response 越界均失败关闭。SelectionToolbar 只允许 Copy 新增放行，尚未迁移的破坏性按钮继续锁定。
>
> 失败史不得删除：Core capture focused 首轮 `1/4`，三项失败因合成 catalog 缺 recipe display names；只补测试目录字段后为 `7/7`，未放宽产品校验。Host 首轮全量 `193/196`，三项新增测试的合成 recipe/stack/lane 组合不在已证明 ordinary 域；只换为既有验证过的普通域夹具后 focused `4/4`、Host library `196/196`、main `2/2`，没有猜测性兼容或 fallback。preload P1 定向中间 `7/8`，lone surrogate 因 NaN 比较漏过，改为显式 surrogate 区间判断后通过。
>
> 当前组合源码只记录新跑的定向结果：Core `816/816`、Host library `196/196` 加 main `2/2`、capture UI focused `103/103`、P1 修复后 capture/desktop Node 组合 `45/45`，TypeScript typecheck 通过。完整 Vitest、完整 native/desktop、production build、完整 E2E、durable E2E 和安装包尚未为这一批 capture/import/export 大改在最终组合源码上重跑，禁止复用下方 direct-deploy 或其他旧切片数字。ordinary import/export 仍在同批开发；GameState v47、envelope v2、cloud schema v8、SQLite layout v3、package 1.2.3 不变，`authorityEligible=false`，未声称部署、签名、发布或灰度完成。
...
>
> Build ID `1.2.3+be80af000295` 的目录包为 **75 文件 / 413,466,490 B**，聚合 SHA-256 `b7c1fcd3030b394d815310f085e1508a77e92bd20e3ca129111af83ba78ff26e`；unsigned ZIP 为 **157,875,293 B**、SHA-256 `cd625a80f3d4a94b153c907c25d5c1461ca845859a9a7b23bb4d305f85651d7f`，Authenticode `NotSigned`。12 秒冒烟中主进程存活可响应、原生 Host 可见、精确清理后残留 0；但临时 `APPDATA/LOCALAPPDATA` 未改变 Electron appData，程序读取既有专用性能版 profile。它未触及 stable profile，但 `temporary-profile-isolation=false`、`smokePassed=false`，不得写成隔离启动通过。保留的一次环境失败是并行 Host 构建的 `STATUS_HEAP_CORRUPTION`；`CARGO_BUILD_JOBS=1` 后全部通过。完整证据及 No-Go 见 [开发报告](./releases/1.2.3-windows-native-plan-completion-be80af0-development-report-2026-08-28.md)。
