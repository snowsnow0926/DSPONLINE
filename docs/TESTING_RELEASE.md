...
> **1.2.5 最终发布前候选（2026-08-30，未部署）**：固定 runtime `0a1c6629ced1c99ba5ec55c4e4947de7300da4bd`，Release ID `1.2.5-0a1c6629ced1`。typecheck、版本一致性、125 个许可证、根/server 生产依赖审计 0、Rust fmt 与 clippy `-D warnings` 通过；Vitest **202 文件通过 / 14 条件跳过，1,711 项通过 / 29 跳过 / 0 失败**；server **385/2** 加 station **4/4**；Ops **56/6**；release-switch **29/29**；native **182/1/0**；Rust **222/222**；最终完整 Chromium **434/27/0**；durable **7/7**；最终 Web production preview **3/3**。Web 为 **1,985 modules**，startup 总 gzip **180,156 B**、JS **86,825 B**、CSS **93,331 B**、最大 JS **58,974 B**、menu **256,162 B**、forbidden **0**。API 182 文件布局、生产依赖与隔离 SQLite health 200 smoke 通过。44.5 MB 真实终局档通过 15×/10 分钟、两次启动、保存读回和 Worker 故障恢复；白矩阵增加 300,540,099,810、火箭 2,239,930,721、施工 622,656，源档未变化。Android `1.2.5 / 1002005` APK/AAB 通过 v2/v3、zipalign、版本和证书连续性；Windows setup/unpacked 的正式 HTTPS 元数据、版本、Build ID 和隔离 12 秒启动通过，残留 0，Authenticode `NotSigned`。source/Web/API/native/download/Windows 组件清单、六归档读回、candidate 12/12、provenance 3/3 和 SHA256SUMS 20/20 均复验，candidate aggregate 为 `9181a35fa9587bb900915a3fb5d40cc44baa80c3971d3c3c7cf000595b927049`。旧 `1.2.5-a347f1889044` 候选已冻结作废。仍缺 Android 实体设备、三档 Windows、Windows 1.2.4→1.2.5 覆盖升级和长时运行；未通过或未获 `1.2.5-0a1c6629ced1` 明确豁免前，生产为 No-Go。详见 [候选记录](./releases/1.2.5-candidate.md) 与 [Release 交接](./RELEASE_HANDOFF_1.2.5.md)。
