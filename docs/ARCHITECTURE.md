...
> **当前发布基线（2026-08-10）**：正式环境当前使用 `1.0.36 / GameState v46`。有效资产的 v43 空间站实验存档不迁入量子共享池，传统物流站升级入口仍作为兼容域命令保留；普通与速通存档按模式隔离。

> **1.0.36 正式基线（2026-08-10）**：设备级新线路并联默认值、可燃冰火电、高密度 Canvas 空间命中，以及传送带/生产/物流运行时索引已发布。正式构建继续使用 `GameState v46 / envelope v2 / cloud schema v7 / SQLite layout v2`，没有数据库或排行榜迁移；两地直接代码回滚为完整 1.0.35。
