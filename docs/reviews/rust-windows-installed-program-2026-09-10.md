...
修复提交 `c9aa106a4753cc8e437dadd7753e05e7c607723d` 已重新构建并冻结，Build ID `1.2.7+c9aa106a4753`，beta / 性能开发版。76 项制品和 79 文件一致；实际 Host/助手集成 36/36、类型、桌面构建、启动预算、thin-UI 与覆盖清单通过。Host `ece575f11977396f8e9687a91045fda8e52a0ddbc0e16876f85c5fe2485443a5`，助手 `2d373bdef5ac353b3b78eb6c1ca05337464f76a467dfb0d81bc5b145bbd62523`，ASAR `024a0556baf34cd28416c92a05049db39d43cf59c4afa98e87505afb5c245812`。构建守护正常 exit 0，111.9395397 秒，最低空闲 5,941,576 KiB，启动 6 GiB/运行 2 GiB 门槛未变。

真实 Electron ASAR 加载与实际助手复验 **PASS**：九字段程序身份与父进程独立冻结信息/文件摘要一致，无资格载体时助手继续返回 `carrier-io`、权威为 false；创建窗口/显示/聚焦/绘制/对话框均为 0，正常 exit 0，无强制清理，隔离 profile 已移除，全部冻结文件前后一致。证据 `package-c9aa106a-catalog-smoke-v2.json`；守护正常 exit 0、无停止原因，3.2119169 秒，最低空闲 8,575,348 KiB。这是实际模块与助手验证，不是从安装 EXE 启动完整游戏或玩家接管验证。

c9aa 的 v1 wrapper 在生成 guard 或探针回执之前退出，未捕获错误，原因未确认；不能算作已执行或通过的探针。已确认它及匹配驱动结束后，v2 在新的证据目录、6/2 GiB 守护下重新派发，并保留 wrapper stdout/stderr。下方 5901 失败及 red/green 记录均保留，不以本节成功抹去。

最终源码专项 51 pass/1 权限 skip，语法/Skill/366 链接检查通过、无新增断链；此前未修正源码另有 50/1 和 377 链接记录。原玩家存档、原工作区及生产系统均未改动。
