...
> **当前生产状态（1.1.0，2026-08-20）**：香港与上海 Web/API 均为 `1.1.0-9b2c579cbe0d` / Build ID `1.1.0+9b2c579cbe0d`，direct previous 均为 `1.0.47-aab581cf0c78`。香港 generation 27（green / 4322），上海 generation 14（blue / 4321）；两地 proxy 均 forward 到当前 API，pending 为空，health/ready 200，schema 8 / SQLite layout 3，API 与 proxy `NRestarts=0`。上海下载页 current 为 `download-site-1.1.0-9b2c579cbe0d`，previous 为 `download-site-1.0.47-aab581cf0c78`；Android stable 为 `1.1.0 / 1001000` 长期证书签名包，Windows stable 为 1.1.0 `NotSigned`。香港 `/canary/previous/` 302 到不可变 `/canary/1.0.47-aab581cf0c78/`；其 Nginx 独立回滚副本为 `dsp-idle-app.conf.pre-previous-fallback-1.0.47-1.1.0`。最终根盘香港约 80%、上海约 88%；香港 offsite timer 已恢复 active，上海既有 offsite timer 仍为 inactive/disabled。完整备份、容量迁移、原子切换、PWA、下载和回滚证据见 [releases/1.1.0.md](./releases/1.1.0.md)。
