...
> **1.1.5 开发侧最终门禁（2026-08-24，尚未发布）**：typecheck/build、125 个许可证、根/server production audit 0；Vitest 178 files passed / 8 skipped、1,457 tests passed / 21 skipped；server 376/2 + station 3/3；ops 56/6；native 25/25；release-switch 29/29；完整 Chromium 427/26/0（453 total，4 workers，6.2 分钟）；durable 7/7；production-preview PWA 1/1。build startup 194,919 B gzip、menu 284,803 B、forbidden startup modules 0。只读 79,443,856 B 玩家附件经稀疏化为 66,298,825 B、gzip 2,763,120 B；768 MiB Renderer 下导入、两次自动保存、手动保存、备份读回与 reload 通过，两次 66.4 MB 规范 JSON 的 Worker gzip 传输约 2.77 MB。派生内存副本的 30 天纯挂机测试总耗时 8.27 秒、输出 66,413,133 B，实体/线路计数和源 SHA 均不变；该档因估算峰值约 5.38 GB 明确走保守路径，不宣称精确 30 天收益。银河 `balanced-log-v2` 客户端/服务端向量、极值和匿名公开 Top 100 重放通过；生产进度延迟快照回退场景专项连续 3/3，随后完整 Chromium 同场景再次通过。Android JVM 首次调用因隔离工作树无 SDK 定位而未执行，正式 release wrapper 必须临时注入已批准 SDK/长期证书并补齐 JVM、签名和元数据门禁。
