...
> **1.1.0 RuntimeWorld 2.0 最终开发门禁（2026-08-20，未发布）**：固定 runtime SHA `9b2c579cbe0de8848f6a4573371fd1322cec0c45`，Build ID `1.1.0+9b2c579cbe0d`，Release ID `1.1.0-9b2c579cbe0d`。root/server `npm ci`、125 包 license 与两处 production audit 通过；typecheck 通过；Vitest 190 files passed / 16 skipped、1,512 passed / 29 skipped / 0 failed；server 363/2 + station 3/3；ops 56/6；release-switch 29/29；native 24/24；durable 7/7；Chromium 429 passed / 28 explicit conditional skipped / 0 failed（457 total，4 workers）；Firefox/WebKit 10/10；production-preview 34/34、PWA 3/3。最终 Web 为 1,972 modules，startup 195,203 B gzip、menu 284,843 B gzip、forbidden 0。连接性能隔离门禁通过；密度性能第一次运行因一个 auto 子轮 P95 33.3 ms 按原阈值失败，独立新进程复跑三轮 P95 16.8/16.7/16.7 ms 后通过，失败记录保留且没有 retry 或阈值放宽。真实档 A/B 的 60 秒精确模拟中位数为 11,397.269 / 2,718.954 ms，较冻结基线下降 47.31% / 58.28%；自动保存 1,222.5 / 370.1 ms，运行/保存均无 Long Task，原件未修改。Desktop 启动 4 进程后精确关闭且 Authenticode `NotSigned`；Android bundle/assemble/lintVital 成功，APK zipalign 成功但 `DOES NOT VERIFY`，AAB 签名条目 0。source 254/254、candidate 10/10、provenance 3/3、SHA256SUMS 12/12；制品扫描 7,930 个条目、运行时内容 5,482 个文件，敏感命中 0。当前生产仍为 1.0.47；没有连接生产、签名、部署或修改下载页。
