...
最终 Windows 包 `1.2.7+ab01bb781ddd` / 驱动 `d77f6eee` 全套 **8/8**，独立 ZIP 读回 **75 件制品**通过，`NotSigned` 离线候选。首轮崩溃恢复测试 7/8 使用了可能过时的租约快照期限；驱动改为进程树终止后等待完整生产 TTL，产品租约/接管规则未改，见下方阶段记录。
