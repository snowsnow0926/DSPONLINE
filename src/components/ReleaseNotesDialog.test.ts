...
import { getCurrentReleaseNotes, getReleaseNotes1039, getReleaseNotes1041, getReleaseNotes1042, getReleaseNotes1043, getReleaseNotes1044, getReleaseNotes1046, getReleaseNotes1047, getReleaseNotesUiCopy } from "../i18n/releaseNotes";
...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY[0].version).toBe("1.1.0");
    expect(RELEASE_NOTES_HISTORY.slice(1).map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 48 }, (_, index) => 47 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 3).map((entry) => entry.version)).toEqual(["1.1.0", "1.0.47", "1.0.46"]);
    expect(getReleaseNotesPage(18, 2).map((entry) => entry.version)).toEqual(["1.0.12", "1.0.11"]);
    expect(getReleaseNotesPage(19, 2).map((entry) => entry.version)).toEqual(["1.0.10", "1.0.9"]);
    expect(getReleaseNotesPage(20, 2).map((entry) => entry.version)).toEqual(["1.0.8", "1.0.7"]);
    expect(getReleaseNotesPage(21, 2).map((entry) => entry.version)).toEqual(["1.0.6", "1.0.5"]);
  });
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.1.0" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.1.0" });
    expect(chinese.items).toHaveLength(5);
    expect(chinese.items.map((item) => item.id)).toEqual(expect.arrayContaining([
      "runtimeworld-compiled-domains",
      "runtimeworld-determinism",
      "runtimeworld-authoritative-save",
    ]));
