...
test("viewport v2 is exposed through direct and bounded transfer IPC without replacing v1", () => {
  ...
  assert.match(main, /\["viewport-v1", "viewport-v2", "factory-read-model-v1", "statistics-v1", "technology-v1"\]\.includes\(request\.projectionType\)/);
...
test("viewport v2 TypeScript contract binds a caller revision and independent cursors", () => {
  ...
  assert.match(desktop, /projectionType:\s*"viewport-v1" \| "viewport-v2" \| "factory-read-model-v1" \| "statistics-v1" \| "technology-v1";/);
});
