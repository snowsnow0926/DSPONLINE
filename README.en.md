...
A 2D infinite-canvas idle factory game inspired by the production flow of Dyson Sphere Program. The current stable release is `1.2.2`, using GameState v47, save envelope v2, cloud schema v8, and SQLite layout v3. It ships as Web/PWA, an Electron desktop app, and a Capacitor Android app, with cloud accounts, four cloud-save slots, and a live leaderboard. Version 1.2.2 gives complex endgame saves a lightweight 3 × 10 simulated-second pure-idle sample followed by material-constrained extrapolation for ordinary production and research; rockets, solar sails, Dyson structures, exports, and contracts remain bounded to the exact 30-second prefix. Hong Kong and Shanghai Web/API, the Shanghai download page, Windows stable, and Android stable all run 1.2.2. Android uses the approved long-term signing certificate; Windows remains explicitly published as `NotSigned` under the established policy. See the [1.2.2 release record](./docs/releases/1.2.2.md) and the [Shanghai download node](https://download.dsponline.cn/).
