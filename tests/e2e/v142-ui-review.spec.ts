...
test("release notes preserve close and acknowledge actions at 360 by 480 and 200 percent", async ({ page }) => {
  ...
  await expect(dialog).toHaveAttribute("aria-label", "经典卡片与画布连线显示修复");
  await expect(dialog.locator(".release-notes-version strong")).toHaveText("1.0.47");
  await expect(dialog.locator(".release-notes-scroll li")).toHaveCount(4);
  await expect(dialog).toContainText("恢复 1.0.43 经典精简卡片");
  await expect(dialog).toContainText("拖动画布后线路继续贴合建筑");
  await expect(dialog).toContainText("连线中的建筑保持清晰");
