...
> **1.2.3 最终整合候选新鲜门禁（2026-08-28，发布前）**：typecheck 通过；完整 Vitest **202 文件通过 / 14 条件跳过，1,704 项通过 / 29 跳过 / 0 失败**；server **384/2** 加 station **4/4**；Ops **56/6**；release-switch **29/29**；Windows 原生/桌面边界 **180 通过 / 1 个 Windows symlink 权限条件跳过 / 0 失败**；Rust workspace **222/222**（core 151、Host 71），fmt 与 clippy `-D warnings` 通过。Chromium 最终 **432/27/0**，durable E2E **7/7**，production preview 的 PWA/版本与 Build ID/发布说明 **3/3**。完整 Chromium 首轮四项失败均来自发布说明改版后旧断言或遗漏关键兼容文案；修正后专项 4/4、完整复跑取得上述最终结果，未隐藏首轮记录。手机浅色 390×844 专项证明“完整检查器”真实正文可见、覆盖桥接层透明且不拦截命中，截图保存于忽略目录 `artifacts/qa/v123-mobile-light-full-inspector-390x844.png`。最终生产构建为 **1,984 modules**，startup 总 gzip **180,069 B**、JS **86,815 B**、CSS **93,254 B**、最大 JS **58,974 B**、menu **254,228 B**、forbidden **0**；125 个运行时许可证与根/server 生产依赖审计均为 0。稳定身份 Windows unpacked 隔离 smoke 为版本 1.2.3、`stable-v1`、历史 `dsp-idle-network` 数据目录、单个原生 Host、正常退出且无残留；正式不可变制品必须在 clean runtime commit 后重新构建，当前测试目录不得直接发布。
