...
3. 16,384 行 × 9 轮合成 pre-gate 中，indexed 与 flat-full 每轮先证明完整结果逐 bit 相等；昂贵 probe 行数为 `9 / 147,456`，debug 中位约 `502 / 1,416 µs`。该数字只说明探测切片，不外推完整模拟秒或真实玩家档。合并后 `planet_metric` 专项 `15/15` 通过，包含 1/5/60、1/10/30 分段、1/2/4/8 worker、同长度 COW、MOD/损坏 shape、失败原子性与 post-barrier writer。
