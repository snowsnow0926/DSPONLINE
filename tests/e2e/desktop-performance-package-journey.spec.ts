...
import { assertSaveReceipt, boundary, continueExisting, deployThroughUi, enterNew, factoryContent, fixture, forceKill, holdNextPersistenceCommit, importFixture, launch, normalClose, productionFixture, profile, records, runDirectory, saveAndVerify, saveTab, sha256 } from "./desktop-journey-helpers";
...
test("close during a controlled in-flight save waits for the durable commit", async () => {
  ...
  try {
    ...
    await (await saveTab(opened.page)).getByLabel("关闭运营中心").click();
    await deployThroughUi(opened.page, "部署风力涡轮机");
    await expect(opened.page.locator(".power-node")).toHaveCount(1);
    ...
    const oldState = inspectSaveEnvelopeChecksum(before.raw!).state! as any;
    const recoveredState = inspectSaveEnvelopeChecksum(recovered.raw!).state! as any;
    expect(recoveredState.entities.filter((entity: any) => entity.buildingId === "wind_turbine")).toHaveLength(1);
    expect(recoveredState.construction.wind_turbine).toBe(oldState.construction.wind_turbine - 1);
    for (const key of ["tray", "planetTrays", "belts", "research", "galaxy", "totalProduced", "elapsedSeconds"]) expect(recoveredState[key]).toEqual(oldState[key]);
...
test("UI places power and smelter, connects ore and produces iron ingots", async () => {
  ...
  try {
    ...
    await deployThroughUi(page, "部署风力涡轮机");
    await expect(page.locator(".power-node")).toHaveCount(1);
    await deployThroughUi(page, "部署电弧熔炉");
...
test("invalid import explicitly rejects and preserves full content and revision", async () => {
  ...
  try {
    ...
    await expect(page.locator(".game-notice").filter({ hasText: /^存档导入失败：/ })).toBeVisible({ timeout: 30000 });
