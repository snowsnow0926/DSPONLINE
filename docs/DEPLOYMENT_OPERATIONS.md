...
> 当前生产状态（2026-08-26，1.1.8 已稳定发布）：香港/上海 Web/API current 均为 `1.1.8-c53497b050c4`，direct previous 均为 `1.1.6-4f6d24f8c709`；香港 generation 39、上海 generation 23，均为 slot blue、pending absent。上海下载 current/previous 为 1.1.8/1.1.6，香港 `/canary/previous/` 302 到不可变 1.1.6。两地 schema v8/layout v3 Backup API evidence、身份、quick_check 和磁盘门禁通过，根盘 83%/74%；API/proxy/timers active、health/ready 200、`NRestarts=0`。下载完整哈希、Range 206、缓存及 PWA 离线/previous 隔离通过。Android 1.1.8 使用批准历史证书；Windows 1.1.8 明确 `NotSigned`。回滚仅按 [1.1.8 正式发布记录](./releases/1.1.8.md) 的现场 generation/evidence/指针边界执行，不能恢复数据库。
