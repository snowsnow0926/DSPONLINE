...
export function NativeBlueprintWorkspace({
  ...
  const queueCancelPendingCopy = queueCancelPending
    ...
    : null;
  const queueFundPendingCopy = queueFundPending
    ? queueFundPending.phase === "dispatching"
      ? "施工领料正在等待 main-owned durable ACK"
      : queueFundPending.phase === "reconciling"
        ? "施工领料结果不确定；仅进行六次有界只读对账，绝不自动重发"
        : queueFundPending.phase === "awaiting-projection"
          ? `施工领料已耐久提交；等待不早于 revision ${queueFundPending.receipt?.revision} 的队列行投影`
          : "施工领料回执或投影无法证明；当前 lineage 保持锁定"
    : null;
  const enqueuePendingCopy = enqueuePending
    ? enqueuePending.phase === "dispatching"
      ? "蓝图入队正在等待 main-owned durable ACK"
      : enqueuePending.phase === "reconciling"
        ? "蓝图入队结果不确定；仅进行六次有界只读对账，绝不自动重发"
        : enqueuePending.phase === "awaiting-projection"
      ...
      {readyFrame.library.length === 0 ? <div className="blueprint-empty"><BoxSelect size={28} /><strong>原生蓝图库为空</strong><span>当前 revision 没有已存储的蓝图记录。</span></div> : readyFrame.library.map((summary) => {
        ...
        return <article className="blueprint-card" key={summary.id} data-native-blueprint-library-id={summary.id}>
          ...
          <footer>
            ...
            {selected ? <button
              type="button"
              disabled={interactionLocked || enqueueSelectionBinding === null}
              onClick={() => {
                if (enqueueSelectionBinding) onBeginQueuePlacement(enqueueSelectionBinding);
              }}
              title={enqueueSelectionBinding
                ? `在原生画布选择${summary.name}的待建位置`
                : `${summary.name}的完整 Rust 详情尚不能证明可进入待建定位`}
        ...
        >
          ...
          <footer>
            ...
            {fundBinding ? <button
              type="button"
              disabled={interactionLocked}
              onClick={() => onSubmitQueueFundIntent(fundBinding, "all")}
              title={`从当前施工托盘为${entry.blueprintName}补充可用建筑、线路和载具；本步骤不会部署`}
              aria-label={`补充全部${entry.blueprintName}`}
              data-native-blueprint-action="fund-queue-all"
              data-native-blueprint-queue-fund={entry.id}
            ><PackageCheck size={14} />补充全部</button> : null}
            <button
              className="danger"
              type="button"
              disabled={interactionLocked}
