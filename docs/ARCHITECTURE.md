...
> **1.1.0 RuntimeWorld 2.0 固定候选（2026-08-20，未发布）**：Simulation Worker 从 GameState v47 编译可丢弃的 `RuntimeWorld`，使用稳定 slot/generation、domain revision、dirty journal 以及传送带、生产/供电、物流/量子领域索引；结构或内容 fingerprint 变化只失效明确领域，revision 缺口、完整性失败和无法证明稳定的依赖回退旧引擎。RuntimeWorld、journal、阻断快照、scratch map、阶段 telemetry 和 UI selector store 都不进入 GameState、envelope、云 payload、排行榜或 IndexedDB records。Simulation Worker 在权威状态旁准备 canonical primary/snapshot envelope，Persistence Worker 独立执行 checksum/proof/CAS/事务提交和精确读回；自动、手动、返回菜单和生命周期 primary 意图共用 `RuntimePrimaryPersistenceLifecycle`。生产默认启用 RuntimeWorld，但不运行 shadow 双跑；Projection v2 与 legacy domain 保留为受控 oracle/fallback。固定 runtime SHA 为 `9b2c579cbe0d...`，完整边界见 [开发报告](./DEVELOPMENT_REPORT_1.1.0.md)。

> **1.1.0 的 1.0.47 线上兼容基线（2026-08-20）**：RuntimeWorld 2.0 在 `aab581cf0c78` 已正式发布行为之上开发。`CanvasDetailPreference` 包含 `classic`，复用 compact LOD 的轻量端口与 1.0.43 `FactoryNodeLodView` 结构，同时使用独立 `224×76` 展示几何；`minimal` 继续使用 `96×32` 一行卡。`CanvasBeltLayer` 的受控 viewport 只在坐标值变化时同步，手势间隔内由 imperative live viewport 保持权威，避免节点几何或选择重渲染把线路单独回拨到陈旧属性。任何 connection draft 活跃时，节点派生与 CSS 兜底都暂停 task/network/line-find dim；结束后原聚焦状态自然恢复。PWA 生成代码中的 `assets/...` 引用从当前 release root 解析，根路由与不可变 canary 路由保持各自作用域。以上均不写入 GameState、存档、云端或确定性哈希。
...
卡片档位拥有稳定的展示几何：一行 `96×32`、经典 `224×76`、中等 `244×118`、数量标记 `88×44` 点击区（可见胶囊约 `80×30`），视口裁剪、Canvas 线路端点和路由中心必须消费当前展示尺寸，不得复用上一档 React Flow measurement。Canvas 批量线路在手势中以 imperative live viewport 为权威；React 节点几何或选择重渲染不得把线路层单独重置为上次发布的陈旧 viewport。带平移/缩放的 `.react-flow__viewport` 禁止 paint containment，否则世界坐标子节点会被裁到变换后的父盒内并出现“线路存在但建筑消失/半截”；只允许不改变绘制与命中的 style containment。完全无节点落入当前视角时，React Flow store 只保留四个边界恢复锚点供 Fit View 计算，权威节点仍在派生层；标准 SVG MiniMap 与低频 CanvasMiniMap 都必须把点击转换为同一世界中心命令。
