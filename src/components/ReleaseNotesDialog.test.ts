...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY.map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 26 }, (_, index) => 25 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 2).map((entry) => entry.version)).toEqual(["1.0.25", "1.0.24"]);
    expect(getReleaseNotesPage(12, 2).map((entry) => entry.version)).toEqual(["1.0.1", "1.0.0"]);
  });
});
