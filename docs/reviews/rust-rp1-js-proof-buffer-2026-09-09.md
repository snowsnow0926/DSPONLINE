...
源码已提交并推送为 `629accab`；新冻结包 75/78 件校验通过，实际 v14 的 9 秒入口仍超原 90 秒、两次正常关闭、原档未变，详见[新包记录](./rust-rp1-629accab-package-2026-09-09.md)。自己的云端仍待终态。863 冻结包 v13 原 90 秒入口失败、诊断 v1 持久读取失败与诊断 v2 的 13 秒样本分别保留，不能拼接成同一包的完整通过。
