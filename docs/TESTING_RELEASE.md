...
> **当前发布基线（2026-08-11）**：当前正式版本使用 `1.0.38 / GameState v46`；有效资产的 v43 空间站实验存档拒绝加载。量子网络回归以 GameState v46 语义为基线，并保留传统物流站升级兼容测试。1.0.38 不升级存档 envelope、云 schema 或 SQLite layout。
...
> 正式原生与生产验收包括 Android 长期证书 v2/v3、API 36.1 模拟器 `1.0.37 → 1.0.38` 原地升级、Windows `NotSigned` setup 隔离启动、两地生产备份副本隔离 API、下载 9/9 完整公网哈希、APK/EXE Range 206、香港/上海/下载页桌面与手机 6 场 Chrome，以及 previous-stable 前后只有当前 1.0.38 worker active、缓存 HTML 哈希不变和正式根页离线重开。用户明确豁免的实体设备与长时门禁及接受的性能风险见 [1.0.38 正式发布记录](./releases/1.0.38.md)。
...
| 单元/领域 | `npm test` | `1.0.38`：107 个文件通过、6 个跳过；950 项通过、18 项跳过 | 引擎暂停边界、递归制造守恒、Worker/稀疏存档与导入边界、单极磁石扩容、科研宏观账本、离线决策、纯挂机幂等与守恒、模式存档隔离、v1-v46 存档、传送带/物流索引 oracle 和云同步等 |
| 浏览器 E2E | `npm run test:e2e` | `1.0.38`：280 项通过、11 项显式条件夹具跳过、0 失败 | Worker 槽位/快照、矿脉扩容、Canvas 有界矩阵、科技树横向输入/多字号、星图批量物流、离线决策、版本公告、速通、纯挂机恢复、模式隔离、新旧手机壳和 IndexedDB 回归 |
| 云服务 | `npm run test:server` | `1.0.38`：70 项通过、2 项可选夹具跳过 | 用户名注册、模式化云槽、schema v3→v7、SQLite layout v1→v2、v35-v46 边界、治理指标、账号安全、历史恢复保护、排行榜完整性和管理员保护 |
