...
mod tests {
    ...
    fn private_macro_v10_credit_roundtrips_without_changing_public_v47() {
        ...
        assert_eq!(restored.pure_idle_macro_construction_carry_seconds(), 0);
    ...
    #[test]
    fn audit_successful_command_clears_macro_construction_carry_from_full_and_dirty_checkpoint() {
        let mut state = CoreState::from_internal_records(
            fixture_identity(7),
            &fixture_records(),
            fixture_catalog(),
        ...
        state
            ...
            .unwrap();
        let manifest: Value = serde_json::from_slice(&full[manifest_key]).unwrap();
        assert!(manifest.get("pureIdleMacroSession").is_none());
        assert!(
            manifest
                .get("pureIdleMacroConstructionCarrySeconds")
                .is_none()
        ...
        let restored =
            CoreState::from_internal_records(fixture_identity(8), &full, fixture_catalog())
                .unwrap();
        assert_eq!(restored.pure_idle_macro_construction_carry_seconds(), 0);
        ...
        let visit = state
            ...
            .unwrap();
        apply_checkpoint_delta(&mut incremental, &visit, delta);
        let manifest: Value = serde_json::from_slice(&incremental[manifest_key]).unwrap();
        assert!(manifest.get("pureIdleMacroSession").is_none());
        assert!(
            manifest
                .get("pureIdleMacroConstructionCarrySeconds")
