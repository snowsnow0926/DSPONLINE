...
import { getCurrentReleaseNotes, getReleaseNotes1039, getReleaseNotes1041, getReleaseNotes1042, getReleaseNotes1043, getReleaseNotes1044, getReleaseNotes1046, getReleaseNotes114, getReleaseNotes115, getReleaseNotes116, getReleaseNotes117, getReleaseNotes118, getReleaseNotes119, getReleaseNotes120, getReleaseNotesUiCopy } from "../i18n/releaseNotes";
...
describe("release notes history", () => {
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 3).map((entry) => entry.version)).toEqual(["1.2.1", "1.2.0", "1.1.9"]);
    expect(getReleaseNotesPage(18, 2).map((entry) => entry.version)).toEqual(["1.0.17", "1.0.16"]);
    expect(getReleaseNotesPage(19, 2).map((entry) => entry.version)).toEqual(["1.0.15", "1.0.14"]);
    expect(getReleaseNotesPage(20, 2).map((entry) => entry.version)).toEqual(["1.0.13", "1.0.12"]);
    expect(getReleaseNotesPage(21, 2).map((entry) => entry.version)).toEqual(["1.0.11", "1.0.10"]);
    expect(getReleaseNotesPage(22, 2).map((entry) => entry.version)).toEqual(["1.0.9", "1.0.8"]);
  });
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.2.1" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.2.1" });
    expect(chinese.items).toHaveLength(6);
    expect(chinese.items.map((item) => item.id)).toEqual(expect.arrayContaining([
      "native-large-save-open",
      "native-summary-cache",
      "native-transaction-memory",
      "unchanged-save-reuse",
      "windows-package-hygiene",
    ]));
