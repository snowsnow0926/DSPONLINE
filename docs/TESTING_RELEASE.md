...
> **单写者与连续产线专项（2026-09-10）**：native:realtime-rpc 经协调器和 IPC 接管后，在 active、paused 及三次进程恢复后逐轮检查旧模拟四种模式、旧命令和两个关闭目标被拒绝，共 35 次；每轮完整状态、主存档/WAL/租约内容不变。保留原 62 秒产线科研、重试、暂停一小时不计时及丢失确认恢复。Rust 单元另查无租约、E1、玩家 prepared/active、损坏租约和同存档别名。真实 Rust 回复与合成授权区分；Electron 事件及浏览器存储仍为合成环境，不能替代实包/IndexedDB/完整准入验收。当前和旧源码结果见[进度报告](./RUST_WINDOWS_FULL_PROGRESS_2026-09-10.md)。
...
> **Windows 隔离会话最终验证（2026-09-10）**：491 源码正常优化 Host 284/4 ignored、两个入口各 3、Native 946/1 skip、Node 174、前端 7/7、类型与严格 Clippy 全过；空目录锁漏洞及固定夹具时钟漂移均已修复，前两轮失败保留。新冻结包 76 制品/79 文件、130/130 打包前检查；实际包内独立会话身份与替换负例通过，零窗口/焦点/弹窗、正常退出，原 0d7 冻结文件未变。证据集中于[本批记录](./reviews/rust-windows-validation-session-2026-09-10.md)，不把历史结果计为当前云端/完整游戏通过；原门槛与完整发布缺口保留。

> **Windows 候选身份验证（2026-09-10）**：最终 Native 931/1 skip、相关检查 161、前端摘要 7/7、类型及漂移通过；Rust 源码/二进制未变，保留同批 Host 278/4 ignored、两入口各 3 及严格 Clippy。0d7 新冻结包 76 制品/79 文件、115/115 打包前检查、实际三方十二字段一致，零窗口/聚焦/弹窗、正常退出；9 个构建步骤原始日志/结果/摘要齐备。7cb 探针失败记录保留。完整实时资格与既有浏览器失败仍未解决，见[本批记录](./reviews/rust-windows-validation-candidate-2026-09-10.md)。
...
> **2026-09-09 施工参考路径后续候选**：已复现关闭批处理的 3 作业参考路径仍构建 11 次计划；改为每作业直接规划后反例通过，原两个计时文件不变，五文件 40/40、类型检查及冻结 1,000 台完整状态对照通过。原量子案例约 1,582.914 ms，完整本机已启动。前一 `38d069ab` 轻量云端 38/0 跳过/1 失败仍保留；新源码云端、构建、浏览器尚待完成。见[记录](./reviews/rust-rp1-js-validation-cost-2026-09-09.md)。

> **2026-09-09 施工材料检查后续候选**：旧复制算法 3 通过 / 1 失败，新检查 4/4、定向 39/39、实际类型检查通过；完整本机 **3,165 通过 / 39 跳过 / 0 失败**，原两份计时测试及 2 秒/5 秒门槛未变。冻结公开来源的新旧逐项和批处理完整状态相同；先前不同建档时间的采样保留为非配对诊断。尚未取得新源码云端/构建/浏览器资格。详见[材料检查段](./reviews/rust-rp1-js-validation-cost-2026-09-09.md)。

> **2026-09-09 空星球模拟候选**：`2501f0d4` 定向 35/35、实际类型检查通过；随后完整本机 3,160 通过 / 39 跳过 / 1 失败，轻量 Windows 云端 34 通过 / 0 跳过 / 1 失败。递归量子建设案例分别 5,110.045 / 5,232.447 ms，仍超过默认 5 秒。完整云端也已结束：Rust 1,363/5 ignored、Native 674/1 跳过，均 0 失败；JS 3,160/39 跳过/1 失败，仍仅建设计时。有限矿脉原 2 秒断言通过，历史失败保留，不放宽门槛。桌面测试只用已验证的隐藏、禁止聚焦、静音离屏入口；本机重任务低优先级串行，余量不足暂缓，禁止关闭用户进程腾资源。详见[结果记录](./reviews/rust-rp1-js-validation-cost-2026-09-09.md)与[后台约束](./reviews/rust-rp1-background-testing-2026-09-09.md)。

> **2026-09-09 大工厂实际后台包入口**：候选 `67a2ffbe` / 基线 `33d96597` 六例同状态、同 5 秒结算三对全部通过，实际继续到运营中心中位 21.003→5.854 秒；完整候选/持久状态/JS 加载重进/第二次重开一致。另两例取消保档及实际保存来源通过；总计 32 次正常关闭 0、窗口显示/聚焦/原生对话框均 0，菜单动画原门槛通过。75 件制品及 78 件冻结文件校验，NotSigned。云端第三轮 Rust/Native 已通过但全量 JS 尚未收口，不授予发布资格，详见[记录](./reviews/rust-rp1-buffered-large-ui-wait-2026-09-09.md)。

> **2026-09-09 本机读取缓冲实际验证**：干净 `6bf9f983` 正常 release Host SHA `1f19d28a…` 构建、冻结及新本地接口/后台策略 **49/0 跳过/0 失败**通过；公开文件导入和运行态候选各三对均完整状态相同、来源不变、正常关闭。对应请求中位 5.177→0.401 秒、16.967→1.758 秒，不是 UI 完整等待。原始终局来源本次被 `offline-macro-time-warp-active` 明确拒绝，记为失败，原档与清理通过。云端原两 JS 文件独立 **31/0 跳过/2 失败**，完整回归仍非全绿；没有放宽时间断言或重建玩家包。见[全部结果](./reviews/rust-rp1-v47-read-buffer-2026-09-09.md)。

> **2026-09-09 第二次读取缓冲云端完整回归**：`056b9b08` 正常优化 Rust **1,363/5 ignored/0**、Native Node **674/1 跳过/0**、fmt、Clippy、Host build 及类型检查通过；完整单元 **3,157/39 跳过/2 失败**，分别为 2 秒宏结算耗时断言与默认 5 秒的建设参考计算期限。两文件本机原样 **32/0 跳过/1 失败**，未调宽断言。原失败保留，下一步独立诊断，并以精确缓存和明确标注的待验 Host 避免重复编译；最终合格制品要求不变。见[全部结果](./reviews/rust-rp1-v47-read-buffer-2026-09-09.md)。

> **2026-09-09 读取缓冲云端及启动诊断**：正常 release Rust workspace **1,363 通过 / 5 ignored / 0 失败**，严格 Clippy、fmt 和 Host 编译通过；首次 Native Node **673 通过 / 1 失败 / 1 跳过**，后续类型及完整游戏单元未执行，没有合格 Host 上传。云端 Add-Type 冷编译实测约 26 秒，原测试的 250 ms 启动宽限不足；恢复 launcher 已有默认宽限后，诊断的六进程退出/临时目录清理通过。修正后本机该完整文件 **15/0/0**，游戏时限及数据断言不变，新的完整云端门禁待执行。见[证据与范围](./reviews/rust-rp1-v47-read-buffer-2026-09-09.md)。

> **2026-09-09 v47 读取缓冲轻量验证**：`840ec68d` 模块 12/12 通过；本 crate 显式 opt-level=0，不能算正常 release 通过或性能测量。测试保留内存保护和低优先级。正常优化构建及回归转由独立 RP1 分支的 Windows 云端验证工作流运行，玩家档留在本机，云端门禁尚待结果。见[精确范围](./reviews/rust-rp1-v47-read-buffer-2026-09-09.md)。

> **2026-09-09 后台实包最终验收**：`33d96597` v4 完成/取消/自然保存三例通过，12 次正常关闭均 0；显示、聚焦和对话框计数均 0，每次真实离屏绘制及原 500 ms 动画调度门槛通过。专项 13/13、完整 Native 工具 674/1 条件跳过/0 失败；75 件包内制品和 78 件冻结文件校验通过。下方待验为历史记录，终局性能未验收，见[最终记录](./reviews/rust-rp1-background-testing-2026-09-09.md)。

> 2026-09-09 离屏调整：原隐藏窗口实包三轮均无显示/聚焦，但动画调度失败，不计全流程通过。后台 v2 改用离屏渲染，专项 13/13、最终完整 Native 工具 674/1 条件跳过/0 失败通过；新包真实隐藏绘制和重开继续验证。日志 background-policy-offscreen-{focused-v1,native-full-v1}.log，见[记录](./reviews/rust-rp1-background-testing-2026-09-09.md)。

> 2026-09-09 后台测试策略：专项 6/6、身份/生命周期组合 26/1 条件跳过、最终完整 Native 工具 674/1 条件跳过/0 失败（两并发）通过。新后台实包待验；启动前必须检查包已含后台策略，禁止用旧可见驱动弹窗或抢焦点。详见[记录](./reviews/rust-rp1-background-testing-2026-09-09.md)。

> **2026-09-09 运行态来源入口最终证据**：运行提交 `dfe0e980`，类型检查、完整单元 3,159/39 跳过/0 失败、完整 Native 工具 668/1 条件跳过/0 失败、完整 Chromium 455/33 跳过/0 失败、构建及门禁通过。Windows beta 本地包三组实际入口通过，12 次正常退出 0；77 件包内文件/80 件冻结文件校验一致，NotSigned。首次清单过期构建失败与标准目录文件锁保留，备用目录 beta 制品独立验证；无新终局完整等待性能结论。见[技术证据](./reviews/rust-rp1-runtime-source-entry-2026-09-09.md)。用户要求后续后台测试：不得再次调用显示窗口或抢焦点的旧桌面驱动。
...
> Rust RP1 恢复回归复验：原全量单元 3,148/39/3，失败文件独立 137/0/0；原全量浏览器 350/33/105，失败场景后续串行及定向复验全部通过，当前独立菜单 p95 277 ms。所有原始失败保留；分批通过不等于新的单次完整全绿。星球重置测试增加真实离线报告确认，不降低数据或交互断言。详见[复验记录](./reviews/rust-rp1-menu-recovery-2026-09-08.md)。
...
> **Rust RP1 统计历史开发验证（2026-09-08，未发布）**：运行源码 `5c619817` 的局部性能对照 30/30 完整字节相同；定向 Rust 13 通过 / 1 性能 ignored，性能项另跑两次通过。标准核心串行 1,099 通过 / 4 ignored，新 Host 差分 66/2 条件跳过，公开 12 场景完整状态一致，Native Node 630/1；类型、构建、边界、fmt 与严格 Clippy 通过。保留 release profile 既有 debug 审计断言失败、一次标准并发堆损坏、两项 Host 目录联接夹具创建失败，不能标记完整 Rust workspace 或发布门禁全绿；逐次复验、完整 JS 合并计数与制品身份见 [阶段记录](./reviews/rust-rp1-history-append-2026-09-08.md)。本轮没有完整浏览器 UI、玩家原生安装包或设备发布资格。
...
上一阶段 Windows 包 `1.2.7+ab01bb781ddd` / 驱动 `d77f6eee` 全套 **8/8**，独立 ZIP 读回 **75 件制品**通过，`NotSigned` 离线候选。首轮崩溃恢复测试 7/8 使用了可能过时的租约快照期限；驱动改为进程树终止后等待完整生产 TTL，产品租约/接管规则未改，见下方阶段记录。

> **1.2.7 Rust 离线边界阶段新鲜证据（2026-09-07，未发布）**：实现提交 `7aba466a` 完整 Rust **1,323 通过 / 3 ignored**、Vitest **3,111/32**；等价 lint 后运行提交 `ab01bb78` 的 Rust 离线专项 **21/21**、严格 Clippy、Native/desktop Node **628/1** 通过。公共目录 10 分钟/8 小时六例物料与关键进度全部通过，三例精确回退完整 canonical 相同；连同 >30 秒自动拒绝为 **7/7**，其余宏路径仍 NOT_QUALIFIED。完整 Chromium 首轮 **441/27/1 失败**，原短时提示用例隔离复跑 **1/1**；未修改共享浏览器运行代码。最终 Host 编译器访问异常一次后以单编译任务原配置重试成功，源码/驱动/实包、首次失败和未跑门禁详见 [阶段记录](./reviews/1.2.7-rust-offline-boundaries-2026-09-07.md)。

> **1.2.7 离线准备候选新鲜证据（2026-09-07，未发布）**：runtime `1c1b80157bdb` 的完整 Vitest **3,111 通过 / 32 跳过**，完整 Chromium **442 通过 / 27 跳过**，最终驱动 `5ef2e7af` 的 Windows 实包旅程 **8/8**。类型、Web/Desktop build、启动预算与 native coverage/thin-UI 检查通过；ZIP 独立解压校验 77 件制品。真实档 Node 函数与实包决定等待各三对，分别缩短 25.1% / 16.1%，完整候选/报告一致或取消后主档不变；未完成离线结算。Rust 公共目录六例安全检查通过，但物料等价仅 3/6、完整状态等价 0/6，资格仍关闭；三例长测试显式运行，默认单元套件跳过。标准打包 EPERM 使用维护的 fallback；两次桌面弹窗驱动失败及修复后 8/8 的日志均保留。Rust 全套、server/Ops、Android 与正式发布门禁没有在本轮完整重跑，见 [范围、失败与证据](./reviews/1.2.7-offline-preparation-performance-2026-09-07.md)。

> **1.2.7 保存/导入候选新鲜证据（2026-09-07，未发布）**：runtime `2b1df8752082` 的最终完整 Vitest **3,092 通过 / 29 条件跳过**；保存、导入、离线、检查点和持久化 Chromium 增量 **94/18**；实包旅程 **8/8**，typecheck、Desktop build、预算与边界检查通过。早先三项改动的完整 Chromium 为 **442/27**，Rust workspace **1,308 通过 / 3 ignored**，fmt/strict Clippy 通过；后续只改共享保存与测试代码。107.6 MB 真实档的 3 对新实包样本全部状态 SHA-256 一致、源文件不变、正常退出，完整导入中位缩短 **27.1%**、最长主线程任务中位缩短 **43.0%**。同步重封装旧候选因主线程回退被拒，未混入最终样本；约 600 秒返回的 `decision-required` 也未计为 Rust 完成。`NotSigned` 离线目录包不代表正式发布，见 [完整范围与证据](./reviews/1.2.7-save-import-performance-2026-09-07.md)。

> **1.2.7 原生恢复阶段新鲜证据（2026-09-07，未发布）**：运行包 `4b13948c8d6d` 的最终桌面旅程 8/8，驱动 `49c5b3d5` 正常关闭再冷重开所得 42/42 恢复样本有效、全进程 Private Bytes 采样覆盖通过。20k / 80k 合成过期侧车检查中位耗时下降 67.9% / 70.5%，匹配恢复基本持平且完整规范状态一致；只证明侧车恢复阶段，不是绝对内存峰值、P95 或整次启动 / 离线生产收益。同批产品源码检查为 Vitest 3061/29、Rust Core 1057/3 ignored + Host 241 + bin 3、native 628/1、Chromium 442/27、durable 7、server 394/2、Ops 56/6，fmt / clippy / typecheck 与 127 包声明通过；计数为通过 / 跳过，0 最终失败。后续提交主要改变测试 / 驱动 / 构建入口，精确适用范围、首次失败与修复见 [阶段记录](./reviews/1.2.7-rust-recovery-stage-2026-09-07.md)。本次仅准备 `NotSigned` Windows 离线性能开发版 candidate；未推送、未公开发布，RP0 / RP1 尚未全面完成，签名、升级、多硬件与长期门禁不继承 1.2.6 豁免。
...
> **1.2.6 历史最终发布候选（2026-08-31）**：固定 runtime `df828869e276e5d3a67513095a4d1c93d13c500f`，Release ID `1.2.6-df828869e276`。产率复制只直结白矩阵科研与逐恒星系戴森终端，不向量子仓库或其他库存复制物品；星图新增三次确认的一键重置星球工厂。typecheck、版本、许可证、生产审计、Rust fmt/clippy 通过；Vitest **1,719/29**，server **385/2 + 4/4**，Ops **56/6**，release-switch **29/29**，native **182/1**，Rust **222/222**。Chromium 435 个有效用例全部通过，其中主轮 2 个并发超时在单 Worker 隔离重跑 2/2；durable **7/7**、production preview **3/3**。44.5 MB 真实终局档完成 30 天 15× 结算并规范重载，源档不变。Android `1.2.6 / 1002006` 长期证书包、Windows `NotSigned` stable 包和 12 件不可变 bundle 已生成；candidate aggregate 为 `56e07009689a3c66f8f39178a9e0aa0e1bb61167be728663e6e8920555aad868`，六归档、provenance 3/3、SHA256SUMS 20/20 均复验。候选阶段尚缺 Android 实体设备、三档 Windows、Windows 1.2.5→1.2.6 覆盖升级和长时运行，随后由用户只针对本 Release ID 明确豁免。本段保留发布前证据；详见 [候选记录](./releases/1.2.6-candidate.md)、[发布交接](./RELEASE_HANDOFF_1.2.6.md) 与 [正式发布记录](./releases/1.2.6.md)。

> **1.2.5 正式发布验收已收口（2026-08-30）**：固定 runtime `0a1c6629ced1c99ba5ec55c4e4947de7300da4bd` 的发布前软件与真实档门禁保持下方记录；用户随后只针对精确 Release ID 明确豁免 Android 实体设备、三档 Windows、Windows 1.2.4→1.2.5 覆盖升级和长时运行。两节点新鲜 SQLite 快照、不可变 staging、dry-run、原子切换、两地 6×稳定采样、严格 TLS 公网读回、PWA 当前 Worker/缓存/离线重载、下载 9/9 完整哈希、APK/setup Range 206、Android v2/v3 证书连续性和 Windows `NotSigned` 身份均通过；previous-stable 已切到不可变 1.2.4 并通过清洁浏览器隔离。完整证据见 [1.2.5 正式发布记录](./releases/1.2.5.md)。

> **1.2.5 历史发布前候选（2026-08-30）**：固定 runtime `0a1c6629ced1c99ba5ec55c4e4947de7300da4bd`，Release ID `1.2.5-0a1c6629ced1`。typecheck、版本一致性、125 个许可证、根/server 生产依赖审计 0、Rust fmt 与 clippy `-D warnings` 通过；Vitest **202 文件通过 / 14 条件跳过，1,711 项通过 / 29 跳过 / 0 失败**；server **385/2** 加 station **4/4**；Ops **56/6**；release-switch **29/29**；native **182/1/0**；Rust **222/222**；最终完整 Chromium **434/27/0**；durable **7/7**；最终 Web production preview **3/3**。Web 为 **1,985 modules**，startup 总 gzip **180,156 B**、JS **86,825 B**、CSS **93,331 B**、最大 JS **58,974 B**、menu **256,162 B**、forbidden **0**。API 182 文件布局、生产依赖与隔离 SQLite health 200 smoke 通过。44.5 MB 真实终局档通过 15×/10 分钟、两次启动、保存读回和 Worker 故障恢复；白矩阵增加 300,540,099,810、火箭 2,239,930,721、施工 622,656，源档未变化。Android `1.2.5 / 1002005` APK/AAB 通过 v2/v3、zipalign、版本和证书连续性；Windows setup/unpacked 的正式 HTTPS 元数据、版本、Build ID 和隔离 12 秒启动通过，残留 0，Authenticode `NotSigned`。source/Web/API/native/download/Windows 组件清单、六归档读回、candidate 12/12、provenance 3/3 和 SHA256SUMS 20/20 均复验，candidate aggregate 为 `9181a35fa9587bb900915a3fb5d40cc44baa80c3971d3c3c7cf000595b927049`。旧 `1.2.5-a347f1889044` 候选已冻结作废。候选阶段尚缺的设备、覆盖升级和长时门禁随后由用户对本 Release ID 明确豁免。

> **1.2.4 开发收口门禁（2026-08-28，未提交、未部署）**：前一收口点 typecheck 通过；完整 Vitest **200 文件通过 / 16 条件跳过，1,671 项通过 / 66 跳过 / 0 失败**；server **385/2** 加 station **4/4**；Ops **56/6**；native JavaScript/桌面边界 **174/7/0**；聚焦 Chromium **37/1/0**；最终完整 Chromium **432/27/0**；durable **7/7**；release-switch **29/29**；production preview **3/3**。Chromium 首轮 **426/28/5** 的 5 项失败均为测试仍断言旧 1.2.3 发布标题、教程版本或公告条目；修正测试基线后定向 **4/4** 和最终全量复跑均通过。随后“内存与积压自动暂停默认关闭”增量通过当前 typecheck、设备偏好/公告 Vitest **7/7** 和 Chromium 设置旅程 **1/1**；最终 clean SHA 仍须重跑完整矩阵。production build 为 **1,985 modules**，startup 总 gzip **180,088 B**、JS **86,834 B**、CSS **93,254 B**、最大 JS **58,974 B**、menu **255,493 B**、forbidden **0**；125 个运行时许可证通过。server 生产依赖审计为 0，根安装报告 0；根独立审计请求因外部服务无响应而有界终止。此证据只属于 dirty 开发工作树；clean runtime SHA、不可变制品、正式签名、实体设备和生产门禁尚未建立，不能据此发布。详见 [开发报告](./DEVELOPMENT_REPORT_1.2.4.md)。

> **1.2.3 最终候选新鲜门禁（2026-08-28）**：固定 runtime `25aeeb34e501fdb68441b88cf305502b730c34d6`。typecheck 通过；完整 Vitest **202 文件通过 / 14 条件跳过，1,704 项通过 / 29 跳过 / 0 失败**；server **384/2** 加 station **4/4**；Ops **56/6**；release-switch **29/29**；Windows 原生/桌面边界 **180/1/0**；Rust workspace **222/222**，fmt 与 clippy `-D warnings` 通过。Chromium 最终 **432/27/0**，durable **7/7**，production preview **3/3**。首轮四项发布文案断言失败已诚实保留，修正后专项与完整复跑通过。手机浅色 390×844 专项证明检查器真实正文可见、桥接层透明且不拦截命中。clean Web 为 **1,984 modules**，startup 总 gzip **180,059 B**、JS **86,805 B**、CSS **93,254 B**、最大 JS **58,974 B**、menu **254,220 B**、forbidden **0**；125 个许可证与根/server 生产审计为 0。正式 Windows setup 为 105,314,618 B、SHA-256 `7f118f230cba796dee99a07ea4b58373e6131a7cccfcc9d20b70cd407f400a93`，stable 身份/正式 URL/12 秒隔离启动通过、残留 0、Authenticode `NotSigned`。Android APK/AAB 为受保护长期证书正式包，APK 5,378,289 B、SHA-256 `8dfae60718f4543fbeb466245056c1fd634ef804253cd326ffa492c27d25cf60`，v2/v3、zipalign、`1.2.3 / 1002003` 与证书连续性通过；API 36.1 模拟器完成正式 1.2.2→1.2.3 原地升级，`firstInstallTime` 保持、Fatal/ANR 0。Web/API/native/download/Windows 组件清单、12 文件 candidate、3 subject provenance 和 20 条 SHA256SUMS 全部复验。硬件缺口仍为 Android 实体设备、三档 Windows、Windows 覆盖升级和长时门禁，未豁免前不得原子切换。

> **E18 + e503 当前整合态最终开发门禁（2026-08-28，未部署）**：clean 运行时提交 `f6923747c69b0be590a2b2e4c0681f1c41ecee75`（Build ID `1.2.3+f6923747c69b`）已通过 typecheck；完整 Vitest **201 文件通过 / 14 条件跳过，1,694 项通过 / 29 跳过 / 0 失败**，最终重点离线/纯挂机/建筑制造与燃料证书回归 **127/127**；server **384/2** 加 station **4/4**，Ops **56/6**；Windows 原生/桌面边界 **179 通过 / 1 个 symlink 权限跳过 / 0 失败**；Rust workspace **220/220**（core 149、Host 71），fmt 与 clippy `-D warnings` 通过。clean desktop build 为 **1,982 modules**，startup 总 gzip **179,922 B**、JS **86,755 B**、CSS **93,167 B**、最大 JS **58,974 B**、menu **253,546 B**、forbidden **0**；Chromium 最终 **431/27/0**，durable E2E **7/7**。Chromium 首轮 **428/27/3** 的三项失败都是 release note 已改而测试仍断言旧文案，修正测试后定向 **3/3**、完整复跑得到上述最终结果，两轮证据均保留。44,167,989 字节玩家档只读纯挂机/施工专项 **3/3**，15×、10 分钟候选恢复白矩阵 335,450,292,523、火箭 7,007,145,985，源文件大小、mtime 与 SHA-256 未变。125 个运行时许可证通过，根与 server 生产依赖审计均为 0 漏洞。Windows x64 目录包为 **75 文件 / 413,286,323 B**，ZIP 为 **157,799,939 B**、SHA-256 `f3c3729b93a290cab861fc9caf8e0816080bd32f695a5d66dda26bc2d873b593`；75/75 ZIP 条目与目录逐文件一致，12 秒启动时主进程可响应、原生 Host 存活，收尾残留为 0。该包 `NotSigned`、没有 installer、没有连接生产或更新源；完整身份见 `artifacts/release-manifests/windows-performance-e18-e503-f6923747c69b-candidate.json`。
...
> **E18 clean Windows 制品门禁（2026-08-28）**：clean 源提交 `460742f8648387f299f31ebd961d2742c5a3ded8` 构建出 Build ID `1.2.3+460742f86483` 的标准 `win-unpacked` 目录包；75 个文件、412,627,614 B，包内 Host SHA-256 为 `032534d525bc2a01116c4bd644ab376b122eaeebab5b603403251ce9288b4804`，与冻结 E18 二进制一致。12 秒隔离 profile 启动时主进程和子进程存活，收尾后残留为 0；ZIP 为 157,603,547 B，SHA-256 `a0b54f712f993541ff733d943e5aed60972f429896ad80c6757e2568e3fd4bc9`。该制品 `NotSigned`、没有 installer、没有连接生产或更新源，不能按稳定版发布。
...
> **1.2.1 Windows 大型存档性能候选（2026-08-27，未部署）**：在 1.2.0 原生存档/影子核心基础上，真实 76.9 MB 档冷启动 `12,870.90 → 8,285.01 ms`，原生 Private Bytes 增量减少 `47,292,416 B`，首次诊断 `12,904.50 → 2,353.27 ms`，同 revision 重复诊断 `0.17 ms`；同 Worker revision 重复分块保存 `1,758 ms / 158 records → 3 ms / 1 record`。完整 Vitest 1,607/27、server 384/2 + station 4/4、ops 56/6、native 35/35、Rust 20/20、长差分 36/36、Chromium 431/27、durable 7/7、production preview 3/3，均 0 失败；真实档投影、保守纯挂机、原生 benchmark 和保存复用各 1/1。production build/startup budget 通过，clean Build ID 的精确 gzip 字节以最终 gate JSON 为准。Android 332 个未签名 Gradle 任务后构建 Windows 包，asar/unpacked Android 残留为 0。JavaScript 仍为权威，第二层 10,000 次故障/升级矩阵及第三层 CORE-390/24 小时多硬件 Gate C 未完成，详见 [1.2.1 开发报告](./releases/1.2.1-windows-performance-development-report-2026-08-27.md)。
