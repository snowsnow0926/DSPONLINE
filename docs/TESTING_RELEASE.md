...
> **1.1.8 稳定发布验收已收口（2026-08-26）**：固定候选 `c53497b050c4…` 的 Vitest **180 文件通过 / 9 跳过；1484 项通过 / 22 跳过**，Chromium **431/26/0**，server **376/2** 加 station **4/4**、ops **56/6**、native **25/25**、release-switch **29/29**、durable WAL **7/7**、PWA preview **1/1**、许可证 **125 包**、TypeScript/build/startup budget 和 root/server 0 漏洞审计通过。生产侧两地不可变清单、备份 evidence、dry-run/原子切换、health/ready、10/10 观察、PWA 离线恢复、previous 隔离、下载完整哈希/Range/缓存均通过。Android 实体设备未执行且未标为通过；Windows 为 `NotSigned`。完整证据与远端归档测试布局限制见 [1.1.8 正式发布记录](./releases/1.1.8.md)。

> **1.1.8 发布候选门禁归档（2026-08-25，已发布）**：独立工作树 `D:\GameDev\DSPidle2-v118-memory-optimization` 的内存闸门新增设备级开关与固定 JS 堆阈值，默认自动 90%，不改变 GameState v47 / envelope v2 / cloud schema v8 / SQLite layout v3。关闭开关现在明确进入“不自动回档”模式：堆水位和模拟积压不会回退到旧检查点，Worker/检查点与分配失败完整性保护仍保留。旧 catalog 无 revision 的首次保存会走协调兼容路径，生命周期监听与自动保存计时器分离，v33 回归从权威持久化后端读取。完整 Vitest **180 文件通过 / 9 跳过；1484 项通过 / 22 跳过**；完整 Chromium **431 通过 / 26 跳过 / 0 失败（457 项）**；server **376/2** 加 station **4/4**、ops **56/6**、native **25/25**、许可证 **125 包**、TypeScript、生产构建和 startup budget 均通过（startup gzip 总计 **179,807 B**）。真实终局存档长时证据见 [1.1.8 长时报告](./releases/1.1.8-long-run-memory-test-2026-08-25.md)。本段只记录发布前开发门禁，生产事实以上述正式记录为准。
