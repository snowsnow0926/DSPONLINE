...
> **Windows runtime 生命周期回归（2026-09-10）**：三个未修复产品负例失败，修复后专项 90、相关 202、完整 Native 981/1 skip 通过；实际助手释放/被终止与 main 类联动通过，但 registry 为 TEST_ONLY，不能计为 Native 活跃模拟。Rust 源码与重建字节保持 5c801a83 已验证值，类型通过，新冻结包待收齐；范围见[本批记录](./reviews/rust-windows-runtime-lifetime-2026-09-10.md)，不改原门槛。
