...
> **1.2.9 正式验收（2026-09-11）**：运行源码 `0521eb63` 的 clean 安装、类型、许可证、构建和长期证书 APK/AAB 通过；CI 单元 3,183/36 跳过，server 390/2 + station 4，Ops 60/2，native Node 631/5，Rust 1,339/3 ignored。本机完整 Chromium 首轮 457/33 跳过/3 失败/1 未运行，原三个文件补验 15/15，覆盖 461 个不同可执行用例；两项补齐版本环境，悬浮交互原断言复验通过，未修改游戏代码或阈值。最终 APK 在新匿名设备完成正式 1.2.8→1.2.9 直接升级、旧 3 个铁矿石保留、新增至 5 后明确保存/冷重开、50 秒离线报告确认。两站公网、下载 r2、旧版/PWA 缓存隔离通过。云端仍有 4 个失败和 2 个 flaky，实体手机、长时/温度/耗电未测，旧 AVD 损坏原因未被本次通过结果解释；详细首次失败、范围与证据见 [1.2.9 发布记录](./releases/1.2.9.md)。以下开发交接是发布前历史。

历史验证补充：[玩家计数恢复](./releases/ops-hk-player-count-recovery-2026-09-08.md)、[玩家展示补偿](./releases/ops-hk-player-display-3700-2026-09-08.md) 与 [Android 1.2.8 热修](./releases/1.2.8-android-cloud-hotfix.md) 的测试和限制保留在各自记录中，不能充当当前源码的新鲜测试结果。

> **1.2.9 开发交接（2026-09-10，未发布）**：冻结源码 `0521eb63` 的 clean Web/APK/AAB、签名连续性、类型、启动预算和原生边界通过；最终 Web/PWA 升级留档通过，交互/恢复 21/21。完整单元在 `780ed5f3` 为 3,183/36 跳过，完整 Chromium 454/33 跳过/6 失败；最终源码 CI 仍在运行。安卓最终重开遇到匿名 AVD 底层库损坏，全新设备在旧版启动阶段触及内存保护，设备验收未完成；未把条件跳过、早期候选或仿真计作最终通过。见 [1.2.9 开发交接](./RELEASE_HANDOFF_1.2.9.md)。
