...
import { getCurrentReleaseNotes, getReleaseNotes1039, getReleaseNotes1041, getReleaseNotes1042, getReleaseNotes1043, getReleaseNotes1044, getReleaseNotes1046, getReleaseNotes114, getReleaseNotes115, getReleaseNotes116, getReleaseNotes117, getReleaseNotes118, getReleaseNotes119, getReleaseNotes120, getReleaseNotes121, getReleaseNotesUiCopy } from "../i18n/releaseNotes";
...
describe("release notes history", () => {
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 3).map((entry) => entry.version)).toEqual(["1.2.2", "1.2.1", "1.2.0"]);
    expect(getReleaseNotesPage(18, 2).map((entry) => entry.version)).toEqual(["1.0.18", "1.0.17"]);
    expect(getReleaseNotesPage(19, 2).map((entry) => entry.version)).toEqual(["1.0.16", "1.0.15"]);
    expect(getReleaseNotesPage(20, 2).map((entry) => entry.version)).toEqual(["1.0.14", "1.0.13"]);
    expect(getReleaseNotesPage(21, 2).map((entry) => entry.version)).toEqual(["1.0.12", "1.0.11"]);
    expect(getReleaseNotesPage(22, 2).map((entry) => entry.version)).toEqual(["1.0.10", "1.0.9"]);
  });
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.2.2" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.2.2" });
    expect(chinese.items).toHaveLength(4);
    expect(chinese.items.map((item) => item.id)).toEqual(expect.arrayContaining([
      "pure-idle-30-second-sample",
      "pure-idle-lightweight-snapshot",
      "pure-idle-material-horizons",
      "pure-idle-terminal-freeze",
    ]));
