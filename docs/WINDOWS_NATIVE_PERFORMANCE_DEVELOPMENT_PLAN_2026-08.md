...
6. identity 切换时组件递增请求 generation 并立即淘汰旧 projection；旧 session/run 的同 revision 回包和旧 commit completion 不得重新出现。设置草稿按 `session/run/registry` scope 保存，因此普通每秒 revision 刷新不会吞掉玩家尚未失焦提交的数字输入；只有 scope 真正切换、明确失败，或 durable ACK 后到达同 scope 的更高完整 projection 才按 Rust 值复位。提交始终绑定当时最新完整 projection 的 revision。Native 与 Web 共用 App 持有的 `operationsTab`，页头“设置”快捷入口会直接打开原生设置页，组件切页也同步同一状态。设置 durable ACK 必须连续到 `R+1`，且 leaf-only receipt 不能带 entity/belt IDs 或 topology dirty；成功 ACK 本身不会解除 pending，只有同 scope 更高 revision 的完整 projection 到达才解锁，明确失败才可立即复位。同步锁阻止 React rerender 前的连续点击重复入队。Web 仍走原 `OperationsWorkspace` 与既有 props/callback，行为不变。
7. 当前组合源码 focused 为 Rust Core `2/2`、Host `3/3`（包含七叶 FIFO、duplicate ACK 与五个 durable fault boundary 冷恢复）、Node/preload/renderer/runtime `125/125`、组件/App/device preference Vitest `10/10`；typecheck、Rust fmt 与 workspace strict clippy 通过。strict clippy 首轮唯一报告是 simulation-speed match 的 `redundant_guards`，改为等价 or-pattern 后通过，没有放宽校验。完整 Vitest、完整 Rust/desktop、production build、E2E、打包、安装、签名与发布未在本切片重跑，不能用这些 focused 数字替代发布门禁。
...
### 24.32 Operations 同 lineage/revision 增量投影（2026-09-01，开发候选）
