...
> **1.2.5 最终发布前候选（2026-08-30，未部署）**：固定 runtime `a347f1889044ccbdd1d6119f686eed6d2eb538c5`，Release ID `1.2.5-a347f1889044`。typecheck、版本一致性、125 个许可证、根/server 生产依赖审计 0、Rust fmt 与 clippy `-D warnings` 通过；Vitest **200 文件通过 / 16 条件跳过，1,673 项通过 / 66 跳过 / 0 失败**；server **385/2** 加 station **4/4**；Ops **56/6**；release-switch **29/29**；native **176/7/0**；Rust **222/222**；最终完整 Chromium **434/27/0**；durable **7/7**；最终 Web production preview **3/3**。Web 为 **1,985 modules**，startup 总 gzip **180,157 B**、JS **86,826 B**、CSS **93,331 B**、最大 JS **58,974 B**、menu **255,914 B**、forbidden **0**。API 182 文件布局、生产依赖与隔离 SQLite health 200 smoke 通过。Android `1.2.5 / 1002005` APK/AAB 由受保护长期证书生成并通过 v2/v3、zipalign、版本和证书连续性；Windows setup/unpacked 的正式 HTTPS 元数据、版本和隔离 12 秒启动通过，残留 0，Authenticode `NotSigned`。source/Web/API/native/download/Windows 组件清单、六归档读回、candidate 12/12、provenance 3/3 和 SHA256SUMS 20/20 均复验，candidate aggregate 为 `d05d744ae91cc0c468a7144869351467c60dbd238b3f16b98e047a20d86b32e0`。首轮旧英文公告断言、Windows rustc 环境崩溃、desktop 覆盖 Web dist 和 bsdtar 中文解码均被门禁发现并在同一 runtime SHA 下修正/重跑，旧清单作废。仍缺 Android 实体设备、三档 Windows、Windows 1.2.4→1.2.5 覆盖升级、长时运行和真实终局档验收；未通过或未获本 Release ID 明确豁免前，生产为 No-Go。详见 [候选记录](./releases/1.2.5-candidate.md) 与 [Release 交接](./RELEASE_HANDOFF_1.2.5.md)。
