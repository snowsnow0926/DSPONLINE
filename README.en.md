...
A 2D infinite-canvas idle factory game inspired by the production flow of Dyson Sphere Program. The online stable release is `1.2.4`, and the current release candidate is `1.2.5`; both use GameState v47, save envelope v2, cloud schema v8, and SQLite layout v3. It ships as Web/PWA, an Electron desktop app, and a Capacitor Android app, with cloud accounts, four cloud-save slots, and a live leaderboard. Version 1.2.5 keeps construction productive in both pure-idle modes, recovers simulation Worker faults without silently rolling back to an older state, and fixes dense blueprint detail layouts. Version 1.2.4 is deployed to Hong Kong and Shanghai Web/API, the download page, Windows stable, and Android stable. Android uses the approved long-term signing certificate; Windows remains explicitly published as `NotSigned` under the established policy. See the [1.2.4 production release record](./docs/releases/1.2.4.md); 1.2.5 is not deployed yet. Downloads remain available from the [Shanghai download node](https://download.dsponline.cn/).
