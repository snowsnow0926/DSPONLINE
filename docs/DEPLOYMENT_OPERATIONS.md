...
> 当前生产状态（2026-08-26，1.1.9 已稳定发布）：香港/上海 Web/API current 均为 `1.1.9-c3f4eff6cb5a`，previous 均为 `1.1.8-c53497b050c4`；香港 generation 40、上海 generation 24，current slot 均为 green。上海下载页 current 为 `download-site-1.1.9-c3f4eff6cb5a`，previous 为 1.1.8；香港 `/canary/previous/` 302 到不可变 1.1.8。两地 API、handoff proxy、health/node-health timer active，`NRestarts=0`，local/public health/ready 200，pending absent；最终根盘香港 54%、上海 74%。两地 pre-1.1.9 Backup API evidence 均通过 SHA-256、quick_check、schema 8/layout 3 和快照身份复验。Android stable 为长期证书签名的 1.1.9 / 1001009，Windows stable 按历史策略为 `NotSigned`；完整下载哈希、Range、PWA/cache、观察证据及精确回滚边界见 [1.1.9 正式发布记录](./releases/1.1.9.md)。代码回滚不得恢复数据库。
