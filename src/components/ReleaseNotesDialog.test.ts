...
import { getCurrentReleaseNotes, getReleaseNotes1039, getReleaseNotes1041, getReleaseNotes1042, getReleaseNotes1043, getReleaseNotes1044, getReleaseNotes1046, getReleaseNotes114, getReleaseNotes115, getReleaseNotes116, getReleaseNotes117, getReleaseNotesUiCopy } from "../i18n/releaseNotes";
...
describe("release notes history", () => {
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 3).map((entry) => entry.version)).toEqual(["1.1.8", "1.1.7", "1.1.6"]);
    ...
    expect(getReleaseNotesPage(22, 2).map((entry) => entry.version)).toEqual(["1.0.6", "1.0.5"]);
  });
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.1.8" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.1.8" });
    expect(chinese.items).toHaveLength(5);
    expect(chinese.items.map((item) => item.id)).toEqual(expect.arrayContaining([
      "memory-auto-pause-policy",
      "chunked-incremental-save",
      "real-save-memory-benchmark",
    ]));
