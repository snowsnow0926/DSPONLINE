...
> **香港运行基线（2026-09-22）**：[1.3.0](./releases/1.3.0.md) 已采用下述量子下载和制造修复，继续使用 JS 权威模拟；Rust 同步实现保留在源码中，未在本次香港 Web 发布中启用。持久协议与 API 版本边界保持兼容。

> **制造中心量子配送修复（2026-09-21，本地开发）**：`quantumLogisticsNetwork` 的下载分配采用“单物品现货可兑现量 → 全局带宽 → 原子扣账”，单物品与全局阶段都保留优先级。可选持久 `downloadCursor` 只在实际受带宽限制的交付边界推进，v47 旧档缺省 0，复制、规范化与导入导出保留它；无需重建库存或升级必填结构。`runtimeFlow.constructionDeliveries` 是模拟侧产生的按中心需求/请求/实收回执，随整个 runtimeFlow 从保存中移除。中心规划用仓库现货选择递归路径，交付前仅做共享预留，不给多个中心复制库存；实际执行仍只消费托盘、WIP 和本中心缓存。JS 与 Rust 实现同步，新增水/中间件/目标跳过的逐秒完整状态差分，原生权威准入不变。

> **1.2.9 候选（2026-09-10）**：Web/Android 菜单保存按 Worker/IndexedDB 能力走完整 proof 持久化，自动快照复用已读回验证的压缩主档；独立校验、fencing/CAS、备份和旧存储兼容路径保留。WASD 仅调用现有视口入口；区域/节点联合移动在一次 GameState 编辑中提交，复用历史补丁和存档协议，不增加持久字段或 Rust 准入。详见 [开发记录](./DEVELOPMENT_1.2.9.md)。
