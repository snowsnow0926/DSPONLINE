...
> 当前生产状态（2026-08-14）：上海 Web/API 运行原始 `1.0.41-32daa4f94380`，香港 Web/API 运行 P0 热修 `1.0.41-2e43f5644241-hk-p0`；上海下载页仍为 `download-site-1.0.38-351c649af9ee`。两地数据库继续独立使用 schema v7 / SQLite layout v2。香港云存档与账号变更维护锁已解除，telemetry 三个 202 边缘熔断继续保留；`/canary/previous/` 302 到不可变 `/canary/1.0.37-853ecdb12795/`。香港切换必须使用 600 秒 readiness 窗口、独立 preflight evidence，并在 handoff 前确认 proxy active/writer 计数为 0；完整证据见 [releases/1.0.41.md](./releases/1.0.41.md)。
