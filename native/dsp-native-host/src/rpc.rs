...
use crate::v47_import::open_v47_import_source;
    ...
    let value = match request {
        ControlRequest::Hello { client_version } => {
            ...
            to_value(HelloResponse {
                ...
                native_format_version: crate::NATIVE_FORMAT_VERSION,
...
pub fn serve(root: PathBuf) -> anyhow::Result<()> {
    serve_with_registry(root, CoreRegistry::default(), response_bytes)
}

fn serve_with_registry(
    root: PathBuf,
    mut cores: CoreRegistry,
    mut encode: impl FnMut(anyhow::Result<HostAction>) -> anyhow::Result<(Vec<u8>, bool)>,
) -> anyhow::Result<()> {
    ...
    while let Some(frame) = read_frame(&mut reader).context("read native host frame")? {
        ...
        let (payload, shutdown) = match request {
            Ok(request) => encode(handle_request(
                &mut store,
                &mut cores,
...
mod tests {
    ...
    // Compiled only into the Rust library test executable, never the shipped
    // Host. The JavaScript integration driver owns this disposable profile.
    // Gameplay, framing, dispatch, WAL and recovery use the production code.
    // Only admission is synthetic, so this is not qualification evidence.
    #[test]
    #[ignore = "TEST_ONLY framed RPC child; requires its isolated integration driver"]
    fn player_authority_rpc_child() {
        let root = PathBuf::from(std::env::var_os("DSP_NATIVE_RPC_TEST_ROOT").unwrap());
        let temporary = std::env::temp_dir().canonicalize().unwrap();
        assert_eq!(root.parent().unwrap().canonicalize().unwrap(), temporary);
        assert!(
            root.file_name()
                .unwrap()
                .to_str()
                .unwrap()
                .starts_with("dsp-native-authority-rpc-test-")
        );
        assert!(!root.symlink_metadata().unwrap().file_type().is_symlink());
        assert_eq!(
            std::fs::read(root.join("TEST_ONLY")).unwrap(),
            b"rpc-integration-v1"
        );
        let mut cores = CoreRegistry::default();
        cores.enable_player_authority_coverage_for_test();
        fn test_admission(value: &mut Value) {
            match value {
                Value::Object(fields) => {
                    if fields.get("stateVersion") == Some(&json!(47))
                        && fields.contains_key("canonicalSha256")
                        && fields.contains_key("domainSha256")
                        && let Some(Value::Object(coverage)) = fields.get_mut("coverage")
                    {
                        coverage.insert("authorityEligible".into(), json!(true));
                    }
                    for child in fields.values_mut() {
                        test_admission(child);
                    }
                }
                Value::Array(values) => values.iter_mut().for_each(test_admission),
                _ => {}
            }
        }
        let result = serve_with_registry(root, cores, |result| {
            let result = result.map(|mut action| {
                match &mut action {
                    HostAction::Continue(value) | HostAction::Shutdown(value) => {
                        test_admission(value)
                    }
                }
                action
            });
        ...
        // Prevent the libtest trailer entering the binary framing stream.
        match result {
            Ok(()) => std::process::exit(0),
            Err(error) => {
                eprintln!("TEST_ONLY RPC child failed: {error:#}");
                std::process::exit(1);
            }
        }
    }
