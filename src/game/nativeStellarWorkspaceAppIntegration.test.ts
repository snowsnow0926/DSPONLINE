...
describe("native stellar workspace App integration", () => {
  ...
  it("binds supported native configuration to exact projected commands while legacy edits retain commitGame", () => {
    ...
    expect(nativeIndustryTag).toContain("onNativeStationMinimumLoadChange={onNativeStationMinimumLoadChange}");
    expect(nativeIndustryTag).toContain("onNativeStationRoutePolicyChange={onNativeStationRoutePolicyChange}");
    expect(nativeIndustryTag).toContain("onNativeStationWarperBudgetChange={onNativeStationWarperBudgetChange}");
    ...
    expect(workspace).toMatch(/工业定位、优先级、装载率、库存上下限及星际路线策略均使用当前投影 revision 的直接命令/);
    expect(workspace).toMatch(/aria-label=\{`\$\{route\.itemLabel\}航线优先级`\}[\s\S]*?disabled=\{!onNativeStationPriorityChange\}/);
    expect(workspace).toMatch(/aria-label=\{`\$\{route\.itemLabel\}最低装载率`\}[\s\S]*?disabled=\{!onNativeStationMinimumLoadChange\}/);
    expect(workspace).toMatch(/aria-label=\{`\$\{route\.itemLabel\}星际路线策略`\}[\s\S]*?disabled=\{route\.targetBuildingId !== "interstellar_logistics_station" \|\| !onNativeStationRoutePolicyChange\}/);
    expect(workspace).toMatch(/aria-label=\{`\$\{route\.itemLabel\}翘曲器预算`\}[\s\S]*?disabled=\{route\.targetBuildingId !== "interstellar_logistics_station" \|\| !onNativeStationWarperBudgetChange\}/);
    ...
    expect(app).toMatch(/onNativeStationMinimumLoadChange=\{[\s\S]*?commitNativeProjectedCommand[\s\S]*?createNativeProjectedStationMinimumLoadCommand/);
    expect(app).toMatch(/onNativeStationRoutePolicyChange=\{[\s\S]*?commitNativeProjectedCommand[\s\S]*?createNativeProjectedStationRoutePolicyCommand/);
    expect(app).toMatch(/onNativeStationWarperBudgetChange=\{[\s\S]*?commitNativeProjectedCommand[\s\S]*?createNativeProjectedStationWarperBudgetCommand/);
