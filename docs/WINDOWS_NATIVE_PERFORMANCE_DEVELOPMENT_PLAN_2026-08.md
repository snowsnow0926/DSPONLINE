...
供电/燃料闭环、量子回放、仿射余数和 renderer 结果边界新增回归后，当前整合源码的新鲜门禁为：TypeScript 通过；完整 Vitest 201 文件通过/14 条件跳过、1,694 项通过/29 跳过/0 失败，最终重点回归 127/127；Server 384/2 加 station 4/4，Ops 56/6；Windows native 179/1/0；Rust workspace 220/220（core 149、Host 71），fmt 与 clippy `-D warnings` 通过。production build 1,982 modules，startup 总 gzip 179,985 B、JS 86,818 B、CSS 93,167 B、最大 JS 58,974 B、menu 253,631 B、forbidden 0；Chromium 最终 431/27/0，durable E2E 7/7；125 个运行时许可证及根/server 生产依赖 0 漏洞通过。Chromium 首轮 428/27/3 的三项失败均为 release note 更新后测试仍断言旧文案，修正测试后定向 3/3、完整复跑 431/27/0；首轮失败不删除。clean Windows 包、隔离启动和最终制品哈希仍需单独执行，且不得复用 E18、e503 或本轮较早源码的身份。
...
这些是当前整合源码的新鲜开发证据，已经覆盖完整 Vitest、server/ops、production build 和完整 E2E，但不代替尚未执行的 clean Windows 包，也不代替 24 小时多硬件、Defender/磁盘故障、签名、覆盖升级、真实云往返或灰度门禁。Windows 诊断制品必须由 clean 源提交生成，并单独记录同一 Build ID、哈希和隔离启动结果。
