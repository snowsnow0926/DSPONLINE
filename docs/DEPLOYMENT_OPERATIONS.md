...
香港、上海 Web/API 与上海下载站均已切换到 `1.0.27-b8e6c0f01ea3` / GameState v46。两地继续使用云 schema v7 和 SQLite layout v2，代码回滚不得恢复数据库；香港 `/downloads/*` 仍重定向上海。1.0.27 Android SHA-256 为 `27b8e60e176b83cc90365eeff5d3f384d0583efff69318a942194fe6702029d5`，Windows SHA-256 为 `a525cd4f7ec5eb7cc81c2b4ee52915fd821bcd08ba28190867c64a48d13eb4f9`（Authenticode `NotSigned` 测试包）。香港发布前 1,752,879,104 字节 Backup API 快照与上海发布前 151,552 字节快照均通过 `quick_check`；Web/API 和下载回滚目标均为 `1.0.26-f675a6a11025`。公网健康、下载页、完整下载哈希、Range、缓存头和 1.0.26 hashed chunk 回退均通过。1.0.27 不含 schema/layout 迁移，为避免重复高 I/O，没有追加香港发布后手工快照；当前根盘约为香港 83%、上海 80%，不得删除当前版、回滚版或有效备份。完整证据见 [releases/1.0.27.md](./releases/1.0.27.md)。
...
香港与上海 `1.0.27-b8e6c0f01ea3` 均为 JS/CSS 启用 gzip，hashed asset 保持 immutable，`index.html` 与 `sw.js` 保持 no-cache。主菜单不 preload `FactoryRuntime`、`flow-vendor`、`game-core` 或 `storage`，英文目录同样只在进入工厂后懒加载；页面加载、LCP 和传输体积按隐私分桶进入受保护后台。
...
Brotli 仍是可选后续项，应先用真实流量比较 CPU、缓存命中和传输节省。不要用“提高服务器配置”替代静态压缩、缓存和 chunk 体积治理；当前 2 核 2 GB 对首版 Node + Nginx + SQLite 足够。1.0.27 发布后历史 SQLite 备份与旧发布目录仍保留，继续按备份保留/异地归档告警运营，且清理不得删除当前版、回滚版或有效备份。
