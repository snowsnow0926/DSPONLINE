...
import { getCurrentReleaseNotes, getReleaseNotes1039, getReleaseNotes1041, getReleaseNotes1042, getReleaseNotes1043, getReleaseNotes1044, getReleaseNotes1046, getReleaseNotes1047, getReleaseNotes1100, getReleaseNotesUiCopy } from "../i18n/releaseNotes";
...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY[0].version).toBe("1.1.1");
    expect(RELEASE_NOTES_HISTORY[1].version).toBe("1.1.0");
    expect(RELEASE_NOTES_HISTORY.slice(2).map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 48 }, (_, index) => 47 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 3).map((entry) => entry.version)).toEqual(["1.1.1", "1.1.0", "1.0.47"]);
    expect(getReleaseNotesPage(18, 2).map((entry) => entry.version)).toEqual(["1.0.13", "1.0.12"]);
    expect(getReleaseNotesPage(19, 2).map((entry) => entry.version)).toEqual(["1.0.11", "1.0.10"]);
    expect(getReleaseNotesPage(20, 2).map((entry) => entry.version)).toEqual(["1.0.9", "1.0.8"]);
    expect(getReleaseNotesPage(21, 2).map((entry) => entry.version)).toEqual(["1.0.7", "1.0.6"]);
  });
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.1.1" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.1.1" });
    expect(chinese.items).toHaveLength(5);
    expect(chinese.items.map((item) => item.id)).toEqual(expect.arrayContaining([
      "station-contract-reoffer",
      "legacy-cloud-save-repair",
      "legacy-cloud-server-compatibility",
      "same-page-save-coordination",
    ]));
