...
> JS 精确模拟的完整机器索引中，星球缺席键明确表示空机器列表，不再回退全实体扫描；机器列表为空或行星没有制造中心时，直接结束对应无对象阶段。保留所有有对象流程和模拟秒数。两项旧代码反例及定向状态检查通过，完整验证待结果，见[空星球阶段记录](./reviews/rust-rp1-js-validation-cost-2026-09-09.md)。
