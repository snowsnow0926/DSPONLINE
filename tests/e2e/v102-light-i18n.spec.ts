...
const RELEASE_NOTE_ID = "2026-08-20-v1.0.47";
...
test("English light release notes are localized and persist dismissal", async ({ page }) => {
  ...
  await page.goto("/?menu=1&lang=en");
  const dialog = page.getByRole("dialog", { name: "Classic Cards and Canvas Connection Fixes" });
  await expect(dialog).toBeVisible();
  await expect(dialog).toContainText("1.0.47");
  await expect(dialog).toContainText("The 1.0.43 classic compact card returns");
  await expect(dialog).toContainText("Belts stay attached after canvas panning");
  await expect(dialog).toContainText("Buildings stay clear while connecting");
