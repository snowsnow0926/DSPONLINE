...
> **Windows 1.1.9 离线存档导出工具（2026-08-27，开发完成，未发布）**：新增只读快照导出命令，覆盖完整主档、最新分块 sidecar、损坏分块回退、跨 1,024 建筑/2,048 线路边界的多区块重建、GZIP 落盘读回、拒绝覆盖和 Electron 形态 IndexedDB 端到端读取；合成源目录导出前后逐字节 SHA-256 一致，输出通过游戏自身 `inspectSave()` / `importGame()` 的 GameState v47 / envelope v2 兼容检查。专项 Node 4/4、兼容专项 1/1、完整 Vitest 1,615/27（197 文件通过/13 跳过）、native 40/40 和 typecheck 均通过。本项没有启动游戏、读取真实玩家存档、部署或修改线上环境；命令和边界见 [Windows 1.1.9 离线存档导出](./WINDOWS_119_OFFLINE_SAVE_EXPORT.md)。
