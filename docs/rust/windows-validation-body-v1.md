...
新增[Windows 验证会话身份](./windows-validation-session-v1.md)：实际目录身份和固定公开夹具由 Host 与 main 独立助手分别读取，复制/更换目录不保持原 profileId。Rust lease 可保持真实句柄，但现有只读进程快照结束即释放锁，尚未成为正文调用方的持续可信上下文；它不切换 profile、不拦截云网络或授予运行资格。

下一步仍是持续会话租约、其余可信上下文、生产者认证、完整验收矩阵、验证专用会话准入与冻结程序内实际单写者交接；扩展玩家、竞速和内容范围须新增对应资格矩阵与合同，不能仅增加允许字符串。
