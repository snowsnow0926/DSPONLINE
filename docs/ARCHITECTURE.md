...
> **1.0.47 画布展示边界（2026-08-20，开发候选，未发布）**：`CanvasDetailPreference` 增加 `classic`，仍复用 compact LOD 的轻量端口与 1.0.43 `FactoryNodeLodView` 结构，但使用独立 `224×76` 展示几何；`minimal` 继续使用 `96×32` 一行卡。`CanvasBeltLayer` 的受控 viewport 只在坐标值变化时同步，手势间隔内由 imperative live viewport 保持权威，避免节点几何或选择重渲染把线路单独回拨到陈旧属性。任何 connection draft 活跃时，节点派生与 CSS 兜底都暂停 task/network/line-find dim；结束后原聚焦状态自然恢复。三项均为可重建设备展示状态，不写入 GameState、存档、云端或确定性哈希。
