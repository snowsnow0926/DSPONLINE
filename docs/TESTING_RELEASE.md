...
> **1.2.3 Windows 原生增量候选门禁（2026-08-27，未部署）**：必须在本工作树重新执行 typecheck、完整 Vitest、server/station、native JavaScript/工具、Rust format/test/clippy、ops、production build、完整 Chromium、durable WAL、真实大档原生差分/保存/导出与 Windows 未签名诊断包。活动脏页失败重试、线路 active/full-scan 严格一致、投影 session/revision/SHA 边界、v47 UTF-16 checksum/重载和 10,000 次故障注入是新增硬门禁。最终精确通过/跳过/失败数量只写入本次 [1.2.3 开发报告](./releases/1.2.3-windows-native-performance-development-report-2026-08-27.md)，不得复用 1.2.2 数字。24 小时、多硬件、Defender/磁盘满、签名和生产灰度仍为独立 Release Agent 阻断项。
