...
> **1.2.3 终局结算内存与施工领域边界（2026-08-27，开发分支，未发布）**：`fast-30s-v3-lite` 与 `pure-idle-macro-v6-construction` 最多保留一个完整校准候选，历史点只保留紧凑库存、累计生产和科研投影；应用尾段前会把采样期的正库存重分布与真实累计生产对账，无法由生产账本支持的正库存变化不得成为长窗收益。普通生产/科研合同落地后，`advanceConstructionAutomationMacroInPlace()` 只推进施工中心领域，复用正式递归计划、科技、材料、目标、批处理与功率规则，不推进生产、物流、戴森、科研或时钟。建筑步骤先将完整输入从托盘/量子缓冲原子转入任务 WIP；旧持久任务缺失中间件时只重建未提交后缀。该领域接口不新增持久字段，候选仍须通过守恒、正式序列化、`inspectSave()` 与重载门禁。

> 时间扭曲兼容近似升级为 `time-warp-lightweight-v4`，用 0.5 秒校准与 0.5 秒独立验证降低终局切片成本；大档 Worker 硬看门狗由 5 秒调整为 15 秒，因为真实 44.3 MB 档的隔离 Worker 切片约 7.4～7.9 秒，主线程仍可立即终止 Worker。普通离线大档标准/受限截止分别为 90/120 秒，低内存保守预算为 180 秒。该调整不降低关键结果误差、数值、守恒、保存或重载门禁。
...
- `src/game/pureIdleMacro.ts`、`pureIdleMacro.worker.ts`、`pureIdleMacroClient.ts`：`pure-idle-macro-v6-construction` 终局宏观纯挂机的校准合同、候选状态、科研账本、施工领域结算、验证摘要和正式重载门禁。启动及恢复从检查点重新计算时间扭曲供电倍率；普通合同不可用、影子尾验偏差、复杂度保护或 Worker 重试时切换有界保守宏观。保守会话从原始合法检查点建立 3 × 10 秒精确轻量校准；候选在墙钟跨过校准边界前仍停留在原检查点，跨过后原子安装唯一校准副本，再以常数时间应用普通生产合同，并由独立施工领域消费真实库存完成递归制造。标准/受限/低内存设备的保守操作现实预算分别为 90/120/180 秒，实际完成即返回，不固定等待。Worker 代次继续隔离迟到消息。`pureIdleRecovery.ts` 将检查点、心跳、墙钟进度、失败次数、开始前暂停状态、结算 ID、检查点指纹、冻结边界、退出原因和提交标记保存到独立 IndexedDB；`inspectPureIdleRecovery()` 明确区分 live、committed、missing、invalid、unavailable，checkpoint 指纹防止把其他槽位或云档接到本地主时间线。Web Lock 与可过期租约防止重复结算，旧 schema v1 记录按读取时默认值兼容，恢复日志不属于 `GameState`、存档 envelope 或云 payload。
...
- 设备级“快速离线结算（实验）”由 `offlineApproximation.ts` 实现，默认开启且不进入 `GameState`。`fast-30s-v3-lite` 对不超过 30 秒的离线时间使用原精确路径；长时间离线在 Worker 内存副本上执行最多 30 个模拟秒的精确校准，只保留一个完整候选与紧凑投影，再以实测合同或保守宏观推进剩余时间，并在有预算时执行短尾验。有限/无限科研由专用整数/BigInt 账本结算，不进入通用仿射字段；施工中心在普通合同落地后由独立权威领域结算。普通库存、缓存和运输量偏差作为诊断，不是快速路径的硬拒绝条件；白糖累计产量、戴森结构点/壳层、火箭/太阳帆和戴森发电功率等关键结果单独尾验。结构、非有限数值、负库存、非法大整数、容量、序列化或正式重载失败都丢弃完整候选。
