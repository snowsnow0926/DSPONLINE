...
For a deliberately scoped Hong Kong-only release, read [references/release-fast-path.md](references/release-fast-path.md). Use [scripts/invoke-protected-release-upload.ps1](scripts/invoke-protected-release-upload.ps1) for hash-checked SSH streaming uploads and `scripts/release-preflight.mjs` / `scripts/create-release-operation-plan.mjs` for the one-time candidate and target plan. These helpers do not switch production pointers; they preserve the same backup, dry-run, health, rollback and secret-handling gates.
