...
describe("release notes history", () => {
  it("keeps the newest release first and exposes bounded pages", () => {
    ...
    expect(RELEASE_NOTES_HISTORY.map((entry) => Number(entry.version.split(".")[2]))).toEqual(Array.from({ length: 48 }, (_, index) => 47 - index));
  ...
  it("supports a small fixed page size without rendering the complete history", () => {
    expect(getReleaseNotesPage(0, 3).map((entry) => entry.version)).toEqual(["1.0.47", "1.0.46", "1.0.45"]);
  ...
  it("serves the current release from stable locale keys", () => {
    ...
    expect(chinese).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.0.47" });
    expect(english).toMatchObject({ id: CURRENT_RELEASE_NOTES.id, version: "1.0.47" });
    expect(chinese.items).toHaveLength(4);
    expect(chinese.items.map((item) => item.id)).toEqual(expect.arrayContaining([
      "classic-canvas-cards",
      "live-canvas-belt-viewport",
      "connection-card-opacity",
    ]));
