...
Hong Kong and Shanghai Web/API run `1.0.37-853ecdb12795` with GameState v46, save envelope v2, cloud schema v7 and SQLite layout v2. Their direct code rollback is `1.0.36-e0ad49062fa3`; Shanghai serves `download-site-1.0.37-853ecdb12795` with the 1.0.36 download directory retained. Android 1.0.37 uses the approved long-term certificate; Windows 1.0.37 remains explicitly `NotSigned`. Production checks confirmed gzip and immutable current/rollback assets, no-cache entry points and feeds, exact 9-file public-download hashes, Range 206, five Chrome smoke scenarios, current PWA offline recovery, active services/timers and `NRestarts=0`. The user waived only this candidate's Android physical-device, low-spec Windows, 1.0.36-to-1.0.37 upgrade-retention and approximately one-hour background gates; do not reuse that waiver or describe it as a pass.

Hong Kong now exposes the direct Web rollback `1.0.36-e0ad49062fa3` through immutable `/canary/1.0.36-e0ad49062fa3/`; `/canary/previous/` redirects there, and the former test compatibility URL redirects to `/canary/previous/`. The retired 1.0.35 public fallback path returns `410`, so an immutable historical URL never silently serves another build. The current Web/API pointers remain `1.0.37-853ecdb12795`. This route is Web-only, uses the current API, has current-worker/Cache Storage/offline-browser isolation evidence, and has an independent Nginx rollback backup.

The Hong Kong database is large enough that an online Backup API run can fail to converge under active writes. Use a low-traffic maintenance window, stop the health timer and service writes, allow at least three minutes for startup health, and verify `quick_check`, schema/layout, mode and hash before mutation. A service restart also triggers an immediate large COS snapshot under the current no-window configuration; keep health timers paused until its state is `ready`, or configure and validate an explicit low-traffic backup window. Current disk usage is approximately 83% on both nodes; keep current, direct rollback and valid backups. Never create a second 3 GB Hong Kong copy on the root filesystem if that would cross the 90% protection threshold. The verified workflow is to escrow the exact backup temporarily in protected object storage, verify its full hash, run smoke against the same local entity, restore the pristine `0600` backup from escrow, reverify the hash, and only then remove escrow. During 1.0.37, COSFS returned `EIO` for Node `copyFile`; the complete encrypted object was recovered through a manual streamed transfer and verified at both ends, but the transport still needs bounded retry, destination hashing and atomic status recovery. Read `docs/releases/1.0.37.md` for exact evidence and `docs/DEPLOYMENT_OPERATIONS.md` for the current procedure.
