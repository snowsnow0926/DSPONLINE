...
> **E18 + e503 当前整合态最终源码门禁（2026-08-28，未部署）**：`D:\GameDev\DSPidle2-windows-native-complete` 的当前整合源码已通过 typecheck；完整 Vitest **201 文件通过 / 14 条件跳过，1,694 项通过 / 29 跳过 / 0 失败**，最终重点离线/纯挂机/建筑制造与燃料证书回归 **127/127**；server **384/2** 加 station **4/4**，Ops **56/6**；Windows 原生/桌面边界 **179 通过 / 1 个 symlink 权限跳过 / 0 失败**；Rust workspace **220/220**（core 149、Host 71），fmt 与 clippy `-D warnings` 通过。production build 为 **1,982 modules**，startup 总 gzip **179,985 B**、JS **86,818 B**、CSS **93,167 B**、最大 JS **58,974 B**、menu **253,631 B**、forbidden **0**；Chromium 最终 **431/27/0**，durable E2E **7/7**。Chromium 首轮 **428/27/3** 的三项失败都是 release note 已改而测试仍断言旧文案，修正测试后定向 **3/3**、完整复跑得到上述最终结果，两轮证据均保留。44,167,989 字节玩家档只读纯挂机/施工专项 **3/3**，15×、10 分钟候选恢复白矩阵 335,450,292,523、火箭 7,007,145,985，源文件大小、mtime 与 SHA-256 未变。125 个运行时许可证通过，根与 server 生产依赖审计均为 0 漏洞。clean 源提交、Windows 重打包、隔离启动和最终制品哈希仍须在下一门禁记录，不能复用下方 E18 或更早候选的制品身份。
