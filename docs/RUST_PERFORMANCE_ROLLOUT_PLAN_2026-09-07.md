...
> **2026-09-11 用户决策：Windows 主导路线已取消。** 后续以 [网页版优先开发计划](./WEB_FIRST_DEVELOPMENT_PLAN_2026-09-11.md) 为当前执行入口，安卓复用共享优化。本文 RP0–RP5 保留为历史工程成果与缺口，不自动继续完整 Windows 原生接管、签名或 Beta 计划；停止投入不等于技术门禁通过。Rust 代码、确定性对照和算法资产保留，可在网页实测支持时用于有界 Wasm 试验。本次没有删除旧代码或更改公开下载。

Role: develop

- 计划 ID：RUST-PERF-20260907，版本 1.5。
...
- RP1 下一步：按真实大档的拒绝原因拆解复合产线，补完整入口、取消、提交与恢复证据；可并行优化已验证有序历史的追加，省去重复排序但保留原桶合并/舍入顺序。当前仅窄铁链取得内核证据，整体长时资格未通过；不能把内核比例当作玩家完整等待或所有存档收益。
