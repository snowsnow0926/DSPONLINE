...
async function installTestBootstrap(page: Page) {
  await page.addInitScript(() => {
    ...
    if (new URLSearchParams(window.location.search).get("releaseNotesTest") !== "1") {
      window.localStorage.setItem("dsp-idle-network.release-notes.seen.v1", "2026-08-20-v1.0.47");
    }
  });
}
...
test("dated release notes appear once and remain available from both settings screens", async ({ page }) => {
  ...
  await expect(releaseNotes).toHaveAttribute("aria-label", "经典卡片与画布连线显示修复");
  await expect(releaseNotes.locator(".release-notes-version strong")).toHaveText("1.0.47");
  await expect(releaseNotes.locator(".release-notes-scroll li")).toHaveCount(4);
  await expect(releaseNotes).toContainText("恢复 1.0.43 经典精简卡片");
  await expect(releaseNotes).toContainText("拖动画布后线路继续贴合建筑");
  await expect(releaseNotes).toContainText("连线中的建筑保持清晰");
  ...
  await expect.poll(() => page.evaluate(() => window.localStorage.getItem("dsp-idle-network.release-notes.seen.v1"))).toBe("2026-08-20-v1.0.47");
  await page.reload();
  await expect(releaseNotes).toHaveCount(0);
