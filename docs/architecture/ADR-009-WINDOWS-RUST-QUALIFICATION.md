...
The [validation-only body v1 contract](../rust/windows-validation-body-v1.md) now has independent main/Host binding implementations consuming authenticated catalog credentials. It binds an independently supplied candidate and isolated no-cloud session, rejects unsupported scope and stale/revoked bodies, and returns no authority grant. Its clock/revocation inputs still require trusted providers, and producer authentication and runtime admission remain pending. See [current verification](../reviews/rust-windows-validation-binding-2026-09-10.md); implementation is not a claim of executed Rust or signed integration results.

The implementation route is specified in [the catalog carrier plan](../rust/windows-qualification-carrier.md): external catalog/member binding, independent main platform helper and Host Windows API verification, explicit no-UI/offline policy, and a separate recovery capability after gameplay qualification expires. The Host has an initial standalone catalog member verifier with bounded locked inputs and negative Windows tests; it is not connected to activation. See [its evidence and remaining work](../reviews/rust-windows-catalog-verifier-2026-09-09.md). The Host now has actual TEST_ONLY signed-member, tamper and trust-removal evidence. A separate readonly Rust helper and main adapter have been implemented with actual rejection tests; they reuse Windows validation code in independent processes. Independent helper signing now has actual cloud evidence; a separately frozen local package has ASAR loading and helper refusal evidence. These are different builds, not a jointly qualified gameplay package. Publisher credentials, authenticated producers, full qualification scope, revocation/clock anti-rollback and actual activation remain open. See [the main helper boundary](../reviews/rust-windows-main-catalog-helper-2026-09-09.md).
