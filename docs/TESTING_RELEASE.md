...
> **当前发布基线（2026-08-11）**：当前正式版本使用 `1.0.37 / GameState v46`；有效资产的 v43 空间站实验存档拒绝加载。量子网络回归以 GameState v46 语义为基线，并保留传统物流站升级兼容测试。1.0.37 不升级存档 envelope、云 schema 或 SQLite layout。

> `1.0.37` clean source `853ecdb12795844c484b1415f8e72967a25e343d` 在 2026-08-11 完成正式门禁并发布：typecheck、Vitest 103 文件通过/6 文件跳过且 936 项通过/17 项跳过、服务端 70/2、运维 6/6、原生工具 8/8、128 个运行时许可证，以及 Playwright 277 项通过/11 条显式条件跳过/0 失败（288 总数，803.5 秒）。根项目与服务端生产依赖审计均为 0。新增矩阵覆盖单极磁石 v20+ 资源目录迁移/人工修复闭环、离线 decision-required/取消/精确重试/普通零收益跳过、科技树标准/紧凑 × 100/150/200% 与多输入方式、星图批量物流宽窄/手机/200% 及成功/跳过原因；1.0.35 的纯挂机与存档模式隔离、1.0.36 的传送带与终局性能套件继续全量执行。两地生产、下载页和 stable 原生 feeds 现均为 1.0.37。
...
| 单元/领域 | `npm test` | `1.0.37`：103 个文件通过、6 个跳过；936 项通过、17 项跳过 | 引擎暂停边界、递归制造守恒、资源目录/人工修复、科研宏观账本、离线决策、纯挂机幂等与守恒、模式存档隔离、v1-v46 存档、传送带/物流索引 oracle 和云同步等 |
| 浏览器 E2E | `npm run test:e2e` | `1.0.37`：277 项通过、11 项显式条件夹具跳过、0 失败 | 科技树横向输入/多字号、星图批量物流、离线决策、版本公告、并联线路、速通、纯挂机恢复、模式隔离、Canvas、新旧手机壳和 IndexedDB 回归 |
| 云服务 | `npm run test:server` | `1.0.37`：70 项通过、2 项可选夹具跳过 | 用户名注册、模式化云槽、schema v3→v7、SQLite layout v1→v2、v35-v46 边界、治理指标、账号安全、历史恢复保护、排行榜完整性和管理员保护 |
...
| 生产构建 | `npm run build` | clean `1.0.37+853ecdb12795` 构建通过 | `tsc -b`、Vite chunk、普通离线/宏观 Worker 和 PWA 资源；生产多 Worker 入口硬关闭 |
| 桌面目录包 | `npm run desktop:pack` | clean 1.0.37 目录包通过；隔离启动 10 秒 4 个进程；`NotSigned` | Electron 启动、包内版本与 Windows 解包 |
| Android release | `npm run android:release` | clean 1.0.37；414 tasks；APK v2/v3、zipalign、批准证书和正式 URL 通过 | applicationId、versionCode、SDK、签名连续性和稳定清单 |
