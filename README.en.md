...
A 2D infinite-canvas idle factory game inspired by the production flow of Dyson Sphere Program. The online stable release is `1.2.6`, using GameState v47, save envelope v2, cloud schema v8, and SQLite layout v3. It ships as Web/PWA, an Electron desktop app, and a Capacitor Android app, with cloud accounts, four cloud-save slots, and a live leaderboard. Version 1.2.6 settles rate-replication gains directly into white-matrix research and per-system Dyson progress without creating inventory items, and adds a three-confirmation planet-factory reset. Hong Kong and Shanghai Web/API, the download page, Windows stable, and Android stable have all been atomically updated; 1.2.5 is previous-stable. Android uses the approved long-term signing certificate; Windows remains explicitly published as `NotSigned` under the established policy. See the [1.2.6 production release record](./docs/releases/1.2.6.md). Downloads remain available from the [Shanghai download node](https://download.dsponline.cn/).
