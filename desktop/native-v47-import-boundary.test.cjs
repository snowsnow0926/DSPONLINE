...
test("renderer-facing v47 import exposes no path or byte payload", () => {
  ...
  assert.match(main, /extensions:\s*\["json", "gz"\]/);
  assert.match(main, /endsWith\("\.json"\)[\s\S]*?endsWith\("\.json\.gz"\)/);
...
test("Rust host advertises import only beside the bounded parser and direct-file guard", () => {
  ...
  assert.match(parser, /parse_v47_envelope_stream/);
  ...
  assert.match(fileGuard, /GzDecoder<BufReader<V47ImportFile>>/);
  assert.match(fileGuard, /trailing data or multiple members/);
