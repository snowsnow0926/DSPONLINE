...
>
> collect/fold 全部成功后才修改候选 runtime；`Arc::make_mut` 使失败候选不能清除源 wake。正式 `CoreState::advance_exact` 只在 `commit_simulated_state` 成功后安装 runtime，外层 campaign/speedrun/commit 失败也不会发布候选。缓存不序列化、不哈希，内存诊断保守计入源/候选 runtime、双 probe buffer、full selection/update buffer 及 B-tree 峰值。测试 flat oracle 直接遍历 `0..entities.len()`，不调用 runtime selection；它和 indexed 路径在 `1/5/60` 秒、`1/10/30` 分段及 1/2/4/8 workers 上保持完整状态与哈希一致。GameState v47、envelope v2、cloud schema v8、SQLite layout v3、package 1.2.3 和 `authorityEligible=false` 不变。
...
>
> 两次真实 belt `transfer_with_bandwidth` 各自产生精确 source/target changed-entity 集合，并分别在前、后 drain 选择前唤醒目标枢纽；后段事件不是前段集合的追加副本，hub 作为 belt source 的真实输出移动也会反向唤醒该 hub。活动集合达到精确 `75%` 时按原持久行序全扫。候选 runtime 只随完整 simulation revision 成功安装，不进入 GameState、WAL、checkpoint、v47 导出、canonical/domain hash；失败候选保留源 wake 与源字节。runtime 保留 topology `Arc` clone，所以后续 `Arc::make_mut` 的同长度/同容量内容修改也必须 COW，pointer identity 必然改变并失败关闭；峰值内存估算保守纳入首个 `BTreeSet` 节点、全行 key payload 与源/候选 runtime Arc COW 双份。
>
> 1,024 hub 合成回归在 `1/5/60` 秒共观察 `2/10/120` 次 drain：冷拍两次均选 `1024`，随后每阶段最多选择 `1`；flat-full 选择对照每次均选 `1024`。该对照绕过 `MaterialDeliveryRuntime::select`，把完整 topology 行直接传入双方共享的旧 drain body；它独立于 indexed selection，但不是第二套独立结算实现。两路完整序列化字节、canonical、domain 与物料守恒 SHA-256 一致。真实 relay→hub、producer→hub 与 hub→sink belt 覆盖前后两段和 source 输出唤醒；60 秒一次 advance 与 `1/10/30` 秒内部步长及对应 `60/6/2` 次提交在先单独断言 revision 差异后，全部其余持久状态字段与摘要一致；1/2/4/8 workers 的 60 秒结果一致。这些是选择形状与等价性证据，不是墙钟收益声明。旧 Core `899/899` 只属于隔离源码 `ab109bc`，不作为 `df47619` 或当前组合树全量证据；组合全量待 root 最终冻结后重跑。
>
...
>
> 该索引不持久化；普通命令、配方/科技/设置/容量/拓扑变化会丢弃 prepared runtime，电源行本身仍按旧路径检查。`matrix_research` 仍逐拍常醒并按持久实体行的旧顺序结算；当前步若可能完成有限科技或无限科研，普通 producer/miner 仍在同一步使用 full-scan oracle，因此科研完成改变的生产、采矿、堆叠或速度倍率可以继续影响排在研究行之后的实体。只有在 belt 输入阶段完成后、科研结算前取得的 session-only `ResearchCompletionBoundaryProof` 证明本步绝不跨科研完成边界时，其他已休眠普通行才继续走 wake index。证明把全部研究所当前矩阵库存作为严格物料上界，同时用“满功率、整拍最高喷涂速度、忽略输入/输出限制、含已有 progress 与向上浮点裕量”计算整数 cycle 上界；任一矩阵不足，或 cycle 上界仍严格小于剩余有限成本总和/当前无限等级成本，才能排除完成。
>
...
>
> **Pure-idle v10 证书准备纵切（2026-09-01）**沿用同一个 `DeterministicRuntime` 池，并把 ordinary 闭合流证书拆成两个固定 join 波次：第一波只读准备有限矿源候选、科研 sink、太阳帆 sink 与火箭 sink；完整 join 后，第二波只读准备依赖第一波私有结果的有限矿账本、仅在帆 sink 存在时启用的动态射线可再生电力证明，以及三窗口 ordinary flow。施工尾段先在调用者边界用显式注入的同一 runtime 完整解析一次实体目录；解析必须 join 后，永久可再生电网、制造中心身份和量子 fingerprint/带宽才以同池并发准备并只读共享该私有快照。partition closure 内不得再次进入 Rayon，也不得调用进程全局第二池。每波必须全部 join，错误仍严格按历史 `source → finite vein → research → sail → renewable → rocket → flow/recipe` 和 `renewable → centers → quantum` 顺序选择；太阳帆仍优先于火箭，最终证书和所有共享库存/戴森写入仍串行。小目录、单 worker 和单一活动域自动串行，绝不跨时间并行，也不从 worker 修改权威 revision。
>
> 4,166 实体、1 条线路的无玩家数据合成目录在 8-worker 诊断中实际得到 wave1 `4/4`、wave2 `2/2`、construction `3/3` selected/observed worker；实体解析闭包在注入的 1/2/4/8-worker runtime 上实际得到 `1/1、2/2、4/4、8/8` selected/observed。修复隐藏全局 runtime 与嵌套 Rayon 后，同一不可变输入的 release prepare-only 15 轮交错 A/B 从 1-worker 中位 `26,646,300 ns` 降至 8-worker `18,288,900 ns`（`-31.364%`）；nearest-rank P95 从 `27,834,100 ns` 降至 `19,883,800 ns`（`-28.563%`），同时通过预先固定的 `3.5%` 中位门禁和无 P95 回退门禁。该数字只包含一次 entity parse、ordinary 两波和 construction prepare，不包含 30 秒精确探针、尾段应用、提交、历史、序列化或磁盘 I/O，不能外推为完整纯挂机或玩家大档加速。原始样本、输入哈希、调度形状、旧证据失效原因和公式记录在 `docs/releases/1.2.3-windows-native-pure-idle-certificate-parallel-evidence-2026-09-01.json`。
