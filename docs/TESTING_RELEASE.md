...
> **1.1.8 发布候选门禁（2026-08-25，未部署）**：独立工作树 `D:\GameDev\DSPidle2-v118-memory-optimization` 的内存闸门新增设备级开关与固定 JS 堆阈值，默认自动 90%，不改变 GameState v47 / envelope v2 / cloud schema v8 / SQLite layout v3。关闭开关现在明确进入“不自动回档”模式：堆水位和模拟积压不会回退到旧检查点，Worker/检查点与分配失败完整性保护仍保留。完整 Vitest **180 文件通过 / 9 跳过；1478 项通过 / 22 跳过**，本次新增的内存决策回归包含在其中；设置文案 E2E **1/1**、延迟 Worker 临界积压不回档 E2E **1/1**、发布说明 E2E **2/2**、TypeScript、生产构建和 startup budget 均通过（本次构建 startup gzip 总计 **179,816 B**）。server **376/2** 加 station **4/4**、ops **56/6**、native **25/25** 和许可证 **125 包**沿用此前候选门禁结果；完整 Chromium 未因本次小范围修复重跑。真实终局存档长时证据见 [1.1.8 长时报告](./releases/1.1.8-long-run-memory-test-2026-08-25.md)。本段只记录发布前开发门禁，不代表已部署线上。
