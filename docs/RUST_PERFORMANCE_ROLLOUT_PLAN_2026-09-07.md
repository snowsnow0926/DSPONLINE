...
- 计划 ID：RUST-PERF-20260907，版本 1.3。
- 编制日期：2026-09-07；依据代码提交：`f7c907408bf2a3500368ef80abd71e111670b85c`。
- 执行状态：**独立原生侧车恢复、共享保存/导入及离线校验/准备阶段已验收；RP0 总矩阵 / RP1 完整离线收益未完成；Rust 实时玩家资格关闭。**
...
- RP1 下一步：公共目录六例物料等价 3/6、完整状态等价 0/6，长时资格仍未通过。可并行处理量子容量满后的物理缓冲/生产尾段、矿耗尽后的在制品、exportWindow 与关联计数推进；之后重做完整差分和真实入口验收。不能把守恒测试通过视为产量或进度等价。
